...
/*yanwen20200623*/
int ixgbe_send_state(struct rte_mbuf *m)
{
    unsigned int idx = (unsigned int)(m->dynfield1[0]);
    struct ixgbe_tx_queue *txq = (struct ixgbe_tx_queue *)(unsigned long)(m->dynfield1[1]);
	if (NULL == txq) {
		return 0;
	}
	...
	if (txq->tx_ring[idx].wb.status & rte_cpu_to_le_32(IXGBE_TXD_STAT_DD)) {
		return 0;
	}
...
ixgbe_xmit_cleanup(struct ixgbe_tx_queue *txq)
{
	...
	/*yanwen20200623*/
	if (sw_ring[desc_to_clean_to].mbuf) {
		sw_ring[desc_to_clean_to].mbuf->dynfield1[0] = txq->nb_tx_desc;
		sw_ring[desc_to_clean_to].mbuf->dynfield1[1] = 0;
	}
	/*yanwen20200623*/
		...
		uint16_t nb_pkts)
{
	...
	for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
		...
		do {
			...
			txe->mbuf = m_seg;
            /*yanwen20200623*/
			m_seg->dynfield1[0] = tx_id;
			m_seg->dynfield1[1] = (uint64_t)(unsigned long)txq;
			/*yanwen20200623*/
			/*
...
static void __attribute__((cold))
ixgbe_tx_free_swring_def(struct ixgbe_tx_queue *txq)
{
	if (txq != NULL &&
	    txq->sw_ring != NULL)
...
static void __attribute__((cold))
ixgbe_reset_tx_queue_def(struct ixgbe_tx_queue *txq)
{
...
static const struct ixgbe_txq_ops def_txq_ops = {
	...
	.free_swring = ixgbe_tx_free_swring_def,
	.reset = ixgbe_reset_tx_queue_def,
};
...
ixgbe_set_tx_function(struct rte_eth_dev *dev, struct ixgbe_tx_queue *txq)
{
	...
	dev->tx_pkt_prepare = ixgbe_prep_pkts;
	/*yanwen20200623*/
	dev->reserved_ptrs[0] = ixgbe_send_state;
	/*yanwen20200623*/
}
			 ...
			 const struct rte_eth_txconf *tx_conf)
{
	...
	//txq->ops->reset(txq);
	ixgbe_reset_tx_queue_def(txq);
    //printf("txq->ops->reset = %p\n", txq->ops->reset);
