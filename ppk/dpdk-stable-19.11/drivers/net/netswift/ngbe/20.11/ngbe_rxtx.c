...
/*
 * ngbe_vmdq_tx_hw_configure - Configure general VMDq TX parameters
...
int
ngbe_rss_conf_init(struct ngbe_rte_flow_rss_conf *out,
...
int
ngbe_action_rss_same(const struct rte_flow_action_rss *comp,
...
int
ngbe_config_rss_filter(struct rte_eth_dev *dev,
		struct ngbe_rte_flow_rss_conf *conf, bool add)
{
	...
	if (!add) {
		if (ngbe_action_rss_same(&filter_info->rss_info.conf,
					  &conf->conf)) {
			ngbe_rss_disable(dev);
			memset(&filter_info->rss_info, 0,
				sizeof(struct ngbe_rte_flow_rss_conf));
			return 0;
		}
		return -EINVAL;
	}
	 ...
	 * the RSS hash of input packets.
	 */
	if ((rss_conf.rss_hf & NGBE_RSS_OFFLOAD_ALL) == 0) {
		ngbe_rss_disable(dev);
		return 0;
	}
	if (rss_conf.rss_key == NULL)
	...
	if (ngbe_rss_conf_init(&filter_info->rss_info, &conf->conf))
		return -EINVAL;
