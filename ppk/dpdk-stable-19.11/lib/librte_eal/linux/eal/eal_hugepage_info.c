...
#define BSP_EQT_FWD_HUGEPAGES    64
static void
calc_num_pages(struct hugepage_info *hpi, struct dirent *dirent)
{
	...
	if (!internal_config.legacy_mem)
		for (i = 0; i < rte_socket_count(); i++) {
            ...
            if(num_pages >= BSP_EQT_FWD_HUGEPAGES)
            {
                num_pages -= BSP_EQT_FWD_HUGEPAGES;
            }
...
{	const char dirent_start_text[] = "hugepages-";
	...
	for (dirent = readdir(dir); dirent != NULL; dirent = readdir(dir)) {
		...
		if (get_hugepage_dir(hpi->hugepage_sz,
            ...
            printf("%s %d num_pages = 0x%x hpi->hugepage_sz = 0x%llx \n",__FUNCTION__,__LINE__,num_pages,hpi->hugepage_sz);
	...
	for (i = 0; i < num_sizes; i++) {
        ...
        printf("%s %d i = %d num_pages = %d hpi->hugepage_sz = 0x%llx \n",__FUNCTION__,__LINE__,i,num_pages,hpi->hugepage_sz );
		if (num_pages > 0)
		{
			return 0;
		}
