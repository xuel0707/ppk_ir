...
static void txgbe_dev_allmulticast_enable(struct rte_eth_dev *dev);
static void txgbe_dev_allmulticast_disable(struct rte_eth_dev *dev);
static int txgbe_dev_link_update(struct rte_eth_dev *dev,
				int wait_to_complete);
static void txgbe_dev_stats_reset(struct rte_eth_dev *dev);
static void txgbe_dev_xstats_reset(struct rte_eth_dev *dev);
...
#else
static void txgbe_dev_interrupt_handler(struct rte_intr_handle *handle,
		void *param);
#endif
...
int txgbe_logtype_init = RTE_LOGTYPE_PMD;
int txgbe_logtype_driver = RTE_LOGTYPE_PMD;

/*
 * The set of PCI devices this driver supports
 */
static const struct rte_pci_id pci_id_txgbe_map[] = {
	{ RTE_PCI_DEVICE(PCI_VENDOR_ID_WANGXUN, TXGBE_DEV_ID_RAPTOR) },
...
/*
 * This function is the same as txgbe_is_sfp() in base/txgbe.h.
 */
static inline int
...
/*
 * This function is based on txgbe_disable_intr() in base/txgbe.h.
 */
static void
...
/*
 * This function is based on code in txgbe_attach() in base/txgbe.c.
 * It returns 0 on success.
 */
static int
eth_txgbe_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	eth_dev->data->mac_addrs = rte_zmalloc("txgbe", ETHER_ADDR_LEN *
	...
	eth_dev->data->hash_mac_addrs = rte_zmalloc("txgbe", ETHER_ADDR_LEN *
						    TXGBE_VMDQ_NUM_UC_MAC, 0);
	if (eth_dev->data->hash_mac_addrs == NULL) {
		PMD_INIT_LOG(ERR,
			     "Failed to allocate %d bytes needed to store MAC addresses",
			     ETHER_ADDR_LEN * TXGBE_VMDQ_NUM_UC_MAC);
		return -ENOMEM;
	}
...
eth_txgbe_dev_uninit(struct rte_eth_dev *eth_dev)
{
	...
	txgbe_fdir_filter_uninit(eth_dev);

	/* remove all the L2 tunnel filters & hash */
	txgbe_l2_tn_filter_uninit(eth_dev);

	/* Remove all ntuple filters of the device */
	txgbe_ntuple_filter_uninit(eth_dev);
		...
		struct rte_pci_device *pci_dev)
{
	return rte_eth_dev_pci_generic_probe(pci_dev,
		sizeof(struct txgbe_adapter), eth_txgbe_dev_init);
}
...
#else /* RTE_VERSION < RTE_VERSION_NUM(17, 5, 0, 0) */
static struct eth_driver rte_txgbe_pmd = {
	.pci_drv = {
		.id_table = pci_id_txgbe_map,
	...
	.eth_dev_init = eth_txgbe_dev_init,
	.eth_dev_uninit = eth_txgbe_dev_uninit,
	.dev_private_size = sizeof(struct txgbe_adapter),
};
 ...
 * Register itself as the [Poll Mode] Driver of PCI TXGBE devices.
 */
static int
rte_txgbe_pmd_init(const char *name __rte_unused, const char *params __rte_unused)
{
	PMD_INIT_FUNC_TRACE();

	rte_eth_driver_register(&rte_txgbe_pmd);
	return 0;
}

static struct rte_driver rte_txgbe_driver = {
	.type = PMD_PDEV,
	.init = rte_txgbe_pmd_init,
};
#endif /* RTE_VERSION < RTE_VERSION_NUM(16, 11, 0, 0) */
...
txgbe_check_mq_mode(struct rte_eth_dev *dev)
{
	...
	if (RTE_ETH_DEV_SRIOV(dev).active != 0) {
		...
		/*
		 * fixme: When DCB/VT is off, maximum number of queues changes
		 */
...
txgbe_dev_start(struct rte_eth_dev *dev)
{
	...
	if (!(hw->subsystem_device_id == TXGBE_DEV_ID_WX1820_KR_KX_KX4) ||
	    !(hw->subsystem_device_id == TXGBE_DEV_ID_RAPTOR_KR_KX_KX4))
			   ...
			   struct txgbe_hw_stats *hw_stats)
{
	...
	//hw_stats->mlfc += rd32(hw, TXGBE_MLFC);
	//hw_stats->mrfc += rd32(hw, TXGBE_MRFC);
	...
	//hw_stats->rx_length_errors += rd32(hw, ABC);
	...
	//hw_stats->rfc += rd32(hw, TXGBE_RFC);
...
/*
 * This function is based on txgbe_update_stats_counters() in txgbe/txgbe.c
 */
static void
...
#endif /* RTE_VERSION >= RTE_VERSION_NUM(16, 7, 0, 0) */
...
txgbe_fw_version_get(struct rte_eth_dev *dev, char *fw_version, size_t fw_size)
{
	...
	hw->phy.get_firmware_version(hw, &etrack_id);
...
/*
 * It reads ICR and sets flag (TXGBE_EICR_LSC) for the link_update.
 *
 * @param dev
 *  Pointer to struct rte_eth_dev.
 *
			   ...
			   struct rte_intr_handle *intr_handle)
{
	...
	if (intr->flags & TXGBE_FLAG_NEED_LINK_UPDATE) {
		...
		/* likely to down */
		else if ((hw->subsystem_device_id & 0xFF) != TXGBE_DEV_ID_KR_KX_KX4)
		...
		if (rte_eal_alarm_set(timeout * 1000,
				      txgbe_dev_interrupt_delayed_handler, (void *)dev) < 0)
...
#else /* RTE_VERSION < RTE_VERSION_NUM(17, 5, 0, 0) */
static void
txgbe_dev_interrupt_handler(struct rte_intr_handle *handle, void *param)
{
	struct rte_eth_dev *dev = (struct rte_eth_dev *)param;

	txgbe_dev_interrupt_get_status(dev, handle);
	txgbe_dev_interrupt_action(dev, handle);
}
#endif /* RTE_VERSION >= RTE_VERSION_NUM(17, 5, 0, 0) */
...
#if RTE_VERSION >= RTE_VERSION_NUM(17, 5, 0, 0)
static int
txgbe_add_rar(struct rte_eth_dev *dev, struct ether_addr *mac_addr,
...
static void
txgbe_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *addr)
{
	struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
...
txgbe_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu)
{
	...
	txgbe_dev_info_get(dev, &dev_info);
...
static uint32_t
txgbe_uta_vector(struct txgbe_hw *hw, struct ether_addr *uc_addr)
{
	uint32_t vector = 0;
...
static int
txgbe_uc_hash_table_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr,
			uint8_t on)
{
	uint32_t vector;
	uint32_t uta_idx;
	...
	if (mirror_conf->rule_type & ETH_MIRROR_VLAN) {
		...
		for (i = 0; i < TXGBE_NUM_POOL; i++) {
			if (mirror_conf->vlan.vlan_mask & (1ULL << i)) {
				 ...
				 * index
				 */
				reg_index = txgbe_find_vlvf_slot(
						hw,
		   ...
		   uint8_t queue, uint8_t msix_vector)
{
	...
	if (direction == -1) {
		...
		/* rx or tx causes */
		//msix_vector |= TXGBE_IVAR_VALID; /* Workround for ICR lost */
...
static u8 *
txgbe_dev_addr_list_itr(__attribute__((unused)) struct txgbe_hw *hw,
...
txgbe_timesync_enable(struct rte_eth_dev *dev)
{
	...
	wr32(hw, TXGBE_ETFLT(TXGBE_ETF_ID_1588),
		ETHER_TYPE_1588 | TXGBE_ETFLT_ENA | TXGBE_ETFLT_1588);
	 ...
	 uint8_t en)
{
	...
	if (mask & ETH_L2_TUNNEL_ENABLE_MASK) {
		if (en)
			ret = txgbe_dev_l2_tunnel_enable(
				dev,
				l2_tunnel->l2_tunnel_type);
		else
			ret = txgbe_dev_l2_tunnel_disable(
				dev,
				l2_tunnel->l2_tunnel_type);
	}
	...
	if (mask & ETH_L2_TUNNEL_INSERTION_MASK) {
		if (en)
			ret = txgbe_dev_l2_tunnel_insertion_enable(
				dev,
				l2_tunnel);
		else
			ret = txgbe_dev_l2_tunnel_insertion_disable(
				dev,
				l2_tunnel);
	}
	...
	if (mask & ETH_L2_TUNNEL_STRIPPING_MASK) {
		if (en)
			ret = txgbe_dev_l2_tunnel_stripping_enable(
				dev,
				l2_tunnel->l2_tunnel_type);
		else
			ret = txgbe_dev_l2_tunnel_stripping_disable(
				dev,
				l2_tunnel->l2_tunnel_type);
	}
	...
	if (mask & ETH_L2_TUNNEL_FORWARDING_MASK) {
		if (en)
			ret = txgbe_dev_l2_tunnel_forwarding_enable(
				dev,
				l2_tunnel->l2_tunnel_type);
		else
			ret = txgbe_dev_l2_tunnel_forwarding_disable(
				dev,
				l2_tunnel->l2_tunnel_type);
	}
...
static const struct eth_dev_ops txgbe_eth_dev_ops = {
...
#if RTE_VERSION < RTE_VERSION_NUM(16, 7, 0, 0)
	PMD_REGISTER_DRIVER(rte_txgbe_driver);
#elif RTE_VERSION < RTE_VERSION_NUM(16, 11, 0, 0)
	PMD_REGISTER_DRIVER(rte_txgbe_driver, txgbe);
	DRIVER_REGISTER_PCI_TABLE(txgbe, pci_id_txgbe_map);
#elif RTE_VERSION < RTE_VERSION_NUM(17, 2, 0, 0)
	RTE_PMD_REGISTER_PCI(net_txgbe, rte_txgbe_pmd.pci_drv);
	...
	RTE_PMD_REGISTER_PCI_TABLE(net_txgbe, pci_id_txgbe_map);
	RTE_PMD_REGISTER_KMOD_DEP(net_txgbe, "* igb_uio | uio_pci_generic | vfio-pci");
	RTE_INIT(txgbe_init_log)
	{
		txgbe_logtype_init = rte_log_register("pmd.net.txgbe.init");
		if (txgbe_logtype_init >= 0)
			rte_log_set_level(txgbe_logtype_init, RTE_LOG_NOTICE);
		txgbe_logtype_driver = rte_log_register("pmd.net.txgbe.driver");
		if (txgbe_logtype_driver >= 0)
			rte_log_set_level(txgbe_logtype_driver, RTE_LOG_NOTICE);
	}
#endif
