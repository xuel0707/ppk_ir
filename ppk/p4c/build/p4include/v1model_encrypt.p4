...
/**
 * encrypt_with_payload is identical in all ways to
 * any control, except that it includes the payload of the packet
 * in the checksum calculation.  The payload is defined as "all bytes
 * of the packet which were not parsed by the parser".
 *
 * Calling encrypt_with_payload is only supported in the
 * ComputeChecksum control.
 */
@noSideEffects
extern void encrypt_with_payload<T>(in T data);
extern void encrypt_with_payload();

@noSideEffects
extern void decrypt_with_payload<T>(in T data);
...
/*
 * The only legal statements in the body of the EncryptWithPayload
 * control are: block statements, calls to the
 * encrypt_with_payload methods, and return statements.
 */
control EncryptWithPayload<H, M>(inout H hdr,
                             inout M meta);
