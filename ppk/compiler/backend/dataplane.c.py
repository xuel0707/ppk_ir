...
#[ extern void parse_packet(packet_descriptor_t* pd, lookup_table_t** tables, parser_state_t* pstate);
...
#[ struct apply_result_s multicast_group_table_apply(STDPARAMS);
#[ struct apply_result_s clone_session_table_apply(enum enum_CloneType type, uint32_t clone_session, SHORT_STDPARAMS);
...
for table in hlir16.tables:
    ...
    for f in sortedfields:
        ...
        if f.width <= 32:
            ...
            #}     }
            #} #endif
            if f.width <= 16 and hi_name == "all_metadatas":
                #[ EXTRACT_INT32_NTOH_PACKET(pd, $href, $fref, *(uint16_t*)key)
                #[ key += ${byte_width};
            elif hi_name == "meta":
                #[ EXTRACT_INT32_NTOH_PACKET(pd, $href, $fref, *(uint32_t*)key)
                #[ key += ${byte_width};
            else:
    ...
    #} }
#[ void multicast_group_table_key(packet_descriptor_t* pd, uint8_t* key) {
#[  #ifdef PPK_DEBUG 
#[      if (unlikely(pd->headers[header_instance_all_metadatas].pointer == NULL)) { 
#[          debug(" " T4LIT(!!!!,error) " " T4LIT(Lookup on invalid header,error) " " T4LIT(all_metadatas,header) "." T4LIT(mcast_grp,field) "\n");
#[      } 
#[  #endif 
#[  EXTRACT_INT32_BITS_PACKET(pd, header_instance_all_metadatas, field_standard_metadata_t_mcast_grp, *(uint32_t*)key) 
#[  key += 2; 
#[  } 
...
# for type in unique_stable(components):
#     #[ void apply_direct_smem_$type($type* smem, uint32_t value, char* table_name, char* smem_type_name, char* smem_name) {
...
for table in hlir16.tables:
    ...
    #{ {
    if hasattr(table, 'key') and len(table.key.keyElements) > 0:
        ...
        for action in table.actions:
            ...
            if action_name == table.default_action.expression.method.path.name:
                continue
        ...
        if hasattr(table, 'direct_counters'):
            for smem in table.direct_meters + table.direct_counters:
                for comp in smem.components:
                    value = "packet_length(pd)" if comp['for'] == 'bytes' else "1"
                    type = comp['type']
                    name  = comp['name']
        ...
        #}    }
        #{     else {
        action_name = table.default_action.expression.method.path.name
        #[           action_code_${action_name}(SHORT_STDPARAMS_IN, entry->action.${action_name}_params);
        #}    }
    else:
        if len(table.actions) > 0 and  hasattr(table, 'default_action') and table.default_action.expression.method.ref.name != table.actions[0].action_object.name:
            action = table.actions[0].action_object.name
        elif hasattr(table, 'default_action'):
            action = table.default_action.expression.method.ref.name
        else :
            action = None
...
#[  { 
#[      uint8_t* key[2]; 
#[      multicast_group_table_key(pd, (uint8_t*)key); 
#[      dbg_bytes(key, table_config[TABLE_multicast_group_table].entry.key_size, 
#[                " " T4LIT(????,table) " Table lookup " T4LIT(multicast_group_table,table) "/" T4LIT(EXACT) "/" T4LIT(%d) ": %s", 
#[                4, 
#[                4 == 0 ? "" T4LIT((empty key),bytes) "" : ""); 
#[      table_entry_multicast_group_table_t* entry = (table_entry_multicast_group_table_t*)exact_lookup(tables[TABLE_multicast_group_table], (uint8_t*)key); 
#[      bool hit = entry != NULL && entry->is_entry_valid != INVALID_TABLE_ENTRY; 
...
#[          if (likely(entry != 0)) { 
#[            switch (entry->action.action_id) { 
#[              case action_set_bitmcast: 
#[                action_code_set_bitmcast(SHORT_STDPARAMS_IN, entry->action.set_bitmcast_params); 
#[                break; 
#[        } 
#[      } 
#[     } 
...
#[      uint8_t key[4]; 
#[
#[      uint32_t val = clone_session;
#[      memcpy(key, &val, 4);
#[      dbg_bytes(key, table_config[TABLE_clone_session_table].entry.key_size, 
#[                " " T4LIT(????,table) " Table lookup " T4LIT(clone_session_table,table) "/" T4LIT(EXACT) "/" T4LIT(%d) ": %s", 
#[                4, 
#[                4 == 0 ? "" T4LIT((empty key),bytes) "" : ""); 
#[      table_entry_clone_session_table_t* entry = (table_entry_clone_session_table_t*)exact_lookup(tables[TABLE_clone_session_table], (uint8_t*)key); 
#[      bool hit = entry != NULL && entry->is_entry_valid != INVALID_TABLE_ENTRY; 
#[      debug("   " T4LIT(??,table) " Lookup " T4LIT(%s,success) ": " T4LIT(%s,action) "%s\n",
#[                hit ? "hit" : "miss", 
#[                entry == NULL ? "(no action)" : action_names[entry->action.action_id], 
...
#[          if (likely(entry != 0)) { 
#[            switch (entry->action.action_id) { 
#[              case action_set_clone_egress_port: 
#[                action_code_set_clone_egress_port(SHORT_STDPARAMS_IN, entry->action.set_clone_egress_port_params); 
#[                break; 
#[        } 
#[      } 
#[     } 
...
for h in hlir16.header_instances:
    if not h.type('type_ref', lambda t: t.is_metadata):
        if "tmp" in h.id:
            ...
            #[ pd->headers[${h.id}].var_width_field_bitwidth = 0;
...
#[     init_headers(pd, tables);
#[     reset_headers(pd, tables);
#[     init_keyless_tables();
    ...
    if hlir16.p4_model == 'V1Switch' and it==1:
        for hdr in hlir16.header_types:
            for fld in hdr.fields:
                if 'ether' in fld.name:
                    #[ #if PPK_VLAN
                    #[ uint32_t res32;
                    #[ (void)res32;
                    #[ MODIFY_INT32_INT32_BITS_PACKET(pd, header_instance_ethernet, field_${hdr.name}_${fld.name}, 0x0081);
                    #[ debug("       v1model :value of ethernet-type field  " T4LIT(% 08x) "\n", GET_INT32_AUTO_PACKET(pd, header_instance_ethernet, field_${hdr.name}_${fld.name}));
                    #[ #endif
        #[ transfer_to_egress(STDPARAMS_IN);
    it = it+1;
    if ctl.name == 'egress':
...
#[         uint8_t* copy_start     = NULL;
#[         if (likely(hdr.was_enabled_at_initial_parse)){
#[             copy_start     = pd->header_tmp_storage + header_instance_infos[hdr.type].byte_offset;
#[         }else {
#[             copy_start     = hdr.pointer;
#[         }
...
#{ {
#[     init_headers(clone_pd, tables);
#[     reset_headers(clone_pd, tables);
#[     int value32;
#[     int res32;
#[     
#[     reset_headers(pd, tables);
...
#[ void handle_clone_packet(STDPARAMS, uint32_t portid)
#{ {
#[     int value32;
#[     int res32;
#[      
...
#[     dbg_bytes(pd->data, packet_length(pd), "\nHandling-- clone --packet (port " T4LIT(%d,port) ", $${}{%02d} bytes)  : ", extract_ingress_port(pd), packet_length(pd));
#[
#[     pd->parsed_length = 0;
#[     parse_packet(pd, tables, pstate);
