...
#define MAX_TTL   				255
#define ENCRY_CARD_DEVICE 		"0000:02:00.0_rsp_s10_sym"
...
int zhijiang_init_crypto_device(void)
{
	...
	ret = add_nexthead(0x8947, GEO_PROTOCOL);
	if(0 != ret)
	{
		MSG_DISPLAY("nexthead tbl add failed!!!");
		return -1;
	...
	ret = add_ethertype(GEO_PROTOCOL, 0x8947);
	if(0 != ret)
	{
		MSG_DISPLAY("ethertype tbl add failed!!!");
		return -1;
 ...
 * �� �� ֵ��  
 *****************************************************************************/
unsigned int IPSec(struct rte_mbuf *in_mbuf, unsigned int spi, uint8_t first_head_offset, uint8_t last_head_length, uint8_t last_head_offset, unsigned int coreid, unsigned int *prt1, unsigned int *prt2, SHORT_STDPARAMS)
{
	/*定义数组copy_head[200]，用于存储不需要加密的包头*/
	unsigned char copy_head[200];
	unsigned char meta_head[MAC_HEAD_LEN+4];
	...
	/* ��ȡ��̫����,����nexthead */
	#if PPK_VLAN
	tmpcur = current + MAC_HEAD_LEN + 4 - 2;
	...
	/* �������ͷ/β��Ϣ */
	#if PPK_VLAN
		memcpy((unsigned char*)meta_head, current, MAC_HEAD_LEN+4);
		/* ָ��ģ̬�ײ� */
		current += (MAC_HEAD_LEN+4);
		current_len -= (MAC_HEAD_LEN+4);
	#else
	...
	/* 确认加密的起始位置并存储不需要加密的包头 */
	int distance_b;
	#if PPK_VLAN
		bool isdistance = (current == (pd -> data + 4 +first_head_offset));
		if(isdistance)
		{
			distance_b = 0;
		}
		...
		{	
			distance_b =(unsigned char*)(pd -> data + 4 + first_head_offset) - current;
			memcpy((unsigned char*)copy_head, current, distance_b);
			current += distance_b;
			current_len -= distance_b;
		}
	#else
		bool isdistance = (current == (pd -> data + first_head_offset));
		if(isdistance)
		{
			distance_b = 0;
		}
		...
		{	
			distance_b =(unsigned char*)(pd -> data + first_head_offset) - current;
			memcpy((unsigned char*)copy_head, current, distance_b);
			current += distance_b;
			current_len -= distance_b;
		}
	...
	*tmpcur = next_head;
	...
	/*添加新的外部包头（加密部分）*/
	#if PPK_VLAN
		int distance_c = (pd -> data + 4 +last_head_offset) - (pd -> data + 4 + first_head_offset) + last_head_length;
		current -= distance_c;
		current_len += distance_c;
		memcpy(current, (pd -> data + 4 + first_head_offset), distance_c);
	#else
		int distance_c = (pd -> data + last_head_offset) - (pd -> data + first_head_offset) + last_head_length;
		current -= distance_c;
		current_len += distance_c;
		memcpy(current, (pd -> data + first_head_offset), distance_c);
	#endif
	...
	if (!isdistance){
		...
		current -= distance_b;
		current_len += distance_b;
		memcpy(current, (unsigned char*)copy_head, distance_b);
	}
	...
	if(open_dbg)
	{
		...
		MSG_DISPLAY("current_len:%d\n", current_len);
		MSG_DISPLAY("distance_c:%d\n", distance_c);
		MSG_DISPLAY("distance_b:%d\n", distance_b);
		// softMemPrint(current, current_len);
	}
	...
	#if PPK_VLAN
		current -= (MAC_HEAD_LEN+4);
		memcpy(current, (unsigned char*)meta_head, MAC_HEAD_LEN+4);
		current_len += (MAC_HEAD_LEN+4);
	#else
		current -= MAC_HEAD_LEN;
	...
	#endif
	/**/
	*prt1 = distance_c + 8 + 16;
	*prt2 = distance_b + distance_c + 8 + 16;
 ...
 * �� �� ֵ��  
 *****************************************************************************/
unsigned int unIPSec(struct rte_mbuf *in_mbuf, unsigned int coreid, uint8_t first_head_offset,uint8_t last_head_length, uint8_t last_head_offset, unsigned int *prt1, unsigned int *prt2, SHORT_STDPARAMS)
{
	/*定义数组，用于存储未加密包头*/
	unsigned char copy_head[200];
	unsigned char meta_head[MAC_HEAD_LEN+4];
	...
	/*获取spi和sequnem*/
	spi = GLUE_GB32(current + distance_b + distance_c);
	seqnum = GLUE_GB32(current + distance_b + distance_c + AH_SEQUENCE_OFFSET);
	...
	hash_data = current + distance_b + distance_c;
	hash_len = current_len - AUTHEN_DATA_LENGTH - distance_b - distance_c;
	...
	/*调整解密后的指针位置和包长度，并重新添加解密过程中被覆盖的未加密部分包头*/
	current += (distance_c + ESP_HEADER_LEN + IV_LEN);
	if(!isdistance)
	{	
		memcpy(current, copy_head, distance_b);
	}
	current_len -= (distance_c + ESP_HEADER_LEN + IV_LEN);
 ...
 */
	...
	/* 封装二层 */
		...
		current -= (MAC_HEAD_LEN+4);
		current_len += (MAC_HEAD_LEN+4);
		memcpy(current, meta_head, (MAC_HEAD_LEN + 4 - MAC_TYPE_LEN));
	#else
		current -= MAC_HEAD_LEN;
		current_len += MAC_HEAD_LEN;
...
void encrypt_with_payload(uint32_t *spi_addr, uint8_t* first_head_offset, uint8_t* last_head_length,uint8_t* last_head_offset,SHORT_STDPARAMS) {
	
	if(spi_addr==NULL){
		MSG_DISPLAY("spi_addr is NULL\n");
		return;
	}
	...
	/* 定义指针*prt1,*prt2，用来更新pd中包头长度的变化*/
	unsigned int prt_1;
	unsigned int prt_2;
	unsigned int *prt1 = &prt_1;
	unsigned int *prt2 = &prt_2;
	unsigned int ret = 0;
	unsigned int core_id;
	unsigned int node;
	unsigned int spi = 0;
	unsigned int firstheadoffset = 0;
	unsigned int lastheadlength = 0;
	unsigned int lastheadoffset = 0;
	int res32; // needed for the macro
	// MSG_DISPLAY("2222222222222222\n");
	memcpy(&spi,spi_addr,4);
	memcpy(&firstheadoffset,first_head_offset,1);
	memcpy(&lastheadlength,last_head_length,1);
	memcpy(&lastheadoffset,last_head_offset,1);
	...
	ret = IPSec(pd->wrapper, spi, firstheadoffset, lastheadlength, lastheadoffset, core_id, prt1, prt2, SHORT_STDPARAMS_IN);
	if(ret){
		MSG_DISPLAY("[%s:%d]\n",__FUNCTION__,__LINE__);
	...
	/* 在pd结构体中同步包信息*/
	for(int i=0;i< HEADER_INSTANCE_COUNT - 2;i++){
		if (pd->headers[i].pointer == NULL || pd->headers[i].pointer > pd->data +  lastheadoffset + lastheadlength){
			continue;
		}else{
			pd->headers[i].pointer -= *prt1;  //*prt1 = distance_c + ESP + IV
		}
	}
	
	// pd->headers[header_instance_geo].pointer -= *prt1;
	// pd->headers[header_instance_gbc].pointer -= *prt1;

	//pd->headers[header_instance_ethernet].pointer -= (20 + 8 + 16);
	//// pd->data = pd->data-(hdrlen+20+8);
	// pd->emit_hdrinst_count = 1;
	// pd->parsed_length = 14 + *prt2;  //*prt2 = distance_b + distance_c + ESP + IV

	//pd->parsed_length = 14+(20 + 8 + 16);//(20 + 8 + 16):ip+esp+iv
	pd->payload_length = pd->wrapper->pkt_len - pd->parsed_length;
	pd->is_emit_reordering = 1;
	MODIFY_INT32_INT32_BITS_PACKET(pd, header_instance_all_metadatas, field_standard_metadata_t_packet_length, packet_length(pd));
...
void decrypt_with_payload(uint8_t* first_head_offset,uint8_t* last_head_length, uint8_t* last_head_offset, SHORT_STDPARAMS){
	/* 定义指针*prt1,*prt2，用来更新pd中包头长度的变化*/
	unsigned int prt_1;
	unsigned int prt_2;
	unsigned int *prt1 = &prt_1;
	...
	memcpy(&lastheadoffset,last_head_offset,1);
	getcpu(&core_id,&node,NULL);
	ret = unIPSec(pd->wrapper, core_id, firstheadoffset,lastheadlength ,lastheadoffset, prt1, prt2, SHORT_STDPARAMS_IN);
	...
	pd->payload_length = pd->wrapper->pkt_len - pd->parsed_length;
	// pd->headers[header_instance_ethernet].pointer +=  *prt1;  //distance_c+ESP+IV
	//pd->headers[header_instance_ethernet].pointer += 44;  //IP+ESP+IV(20+8+16)
	
	pd->data = pd->data + *prt1;
	//pd->data = pd->data+44;
	if(ret){
		MSG_DISPLAY("[%s:%d]\n",__FUNCTION__,__LINE__);
