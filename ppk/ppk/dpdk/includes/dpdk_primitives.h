...
#define EXTRACT_INT32_NTOH_PACKET(pd , h, f, dst) EXTRACT_INT32_NTOH(handle(header_desc_ins(pd , h), f), dst)
...
#define LOOKAHEAD(len,lkahead_data) (len<=8?(uint8_t)lkahead_data:rte_be_to_cpu_16((uint16_t)lkahead_data))
...
static int set_field(fldT f[], bufT b[], uint32_t value32, int bit_width) {
    ...
    if (f != 0) {
        fldT fld = f[0];
        debug("    " T4LIT(=,field) " Modifying field " T4LIT(%s,header) "." T4LIT(%s,field) "/" T4LIT(%d) "b (" T4LIT(%d) "B) = " T4LIT(%u) " (0x" T4LIT(%0*x) ")\n",
              header_instance_names[fld.hdr],
              field_names[fld.fld],
              bit_width,
              bit_width <= 8 ? 1 : bit_width <= 16 ? 2 : 4,
              bit_width <= 8 ? (uint8_t)value32 : bit_width <= 16 ? (uint16_t)value32 : (uint32_t)value32,
              (bit_width+7)/8*2,
              bit_width <= 8 ? (uint8_t)value32 : bit_width <= 16 ? (uint16_t)value32 : (uint32_t)value32);
        int res32;
        if(fld.pd == header_instance_meta) {
