...
void fill_gbc_exact_table(uint16_t isvalid[6],uint8_t flag,uint8_t port,uint32_t bitcast){
    char buffer[2048]; 
    struct p4_header* h;
    ...
    strcpy(te->table_name, "IngressImpl.gbc_exact_0");
    ...
    FILE *fp6;
    fp6 = fopen("/home/aa/logs/aaa.txt", "a");
    if (fp6)
    {
        fprintf(fp6, "flag:{%d}", flag);
        fprintf(fp6, "\n");
        fclose(fp6);
    }
    ...
    if(flag == 0){
        ...
        strcpy(a->description.name,"IngressImpl.geo_multicast");
    ...
    if(flag == 1){
        ...
        strcpy(a1->description.name,"IngressImpl.geo_unicast");
...
void fill_dest_guid_exact_table(uint8_t dest_guid[4],uint8_t nxtHopMac[6],uint8_t port)
{
    ...
    h = create_p4_header(buffer, 0, 2048);
    te = create_p4_add_table_entry(buffer,0,2048);
    strcpy(te->table_name, "IngressImpl.dest_guid_exact_0");
    ...
    a = add_p4_action(h, 2048);
    strcpy(a->description.name, "IngressImpl.dest_guid_forward");
...
void fill_powerlink_exact_table(uint8_t dstmac[6],uint8_t port_num)
{
    char buffer[2048]; 
    struct p4_header* h;
    struct p4_add_table_entry* te;
    ...
    h = create_p4_header(buffer, 0, 2048);
    te = create_p4_add_table_entry(buffer,0,2048);
    strcpy(te->table_name, "IngressImpl.powerlink_exact_0");
    ...
    a = add_p4_action(h, 2048);
    strcpy(a->description.name, "IngressImpl.epl_set_egress_port");
...
void set_default_action_gbc_exact()
{
    char buffer[2048];
    struct p4_header* h;
    struct p4_set_default_action* sda;
    ...
    sda = create_p4_set_default_action(buffer,0,sizeof(buffer));
    strcpy(sda->table_name, "IngressImpl.gbc_exact_0");

    a = &(sda->action);
    strcpy(a->description.name, "geo_fwd2ONOS");
...
void set_default_action_dest_guid_exact()
{
    char buffer[2048];
    struct p4_header* h;
    struct p4_set_default_action* sda;
    ...
    sda = create_p4_set_default_action(buffer,0,sizeof(buffer));
    strcpy(sda->table_name, "IngressImpl.dest_guid_exact");
...
void set_default_action_powerlink_exact()
{
    char buffer[2048];
    struct p4_header* h;
    struct p4_set_default_action* sda;
    ...
    sda = create_p4_set_default_action(buffer,0,sizeof(buffer));
    strcpy(sda->table_name, "IngressImpl.powerlink_exact_0");
...
void init_simple() {
    ...
    // uint8_t ttl = 0;
    // uint8_t hoplimit = 0;
	// fill_gbc_exact_table(ip2,mask,dstmac,port);
...
int read_config_from_file(char *filename)
{
	...
	while (fgets(line,sizeof(line),f)){
		...
		if (line[0] == 'G'){
			...
			if (8 == sscanf(line,"%c %hhx:%hhx:%hhx:%hhx:%hhx:%hhx %hhd",&dummy,&dstmac[0],&dstmac[1],&dstmac[2],&dstmac[3],&dstmac[4],&dstmac[5],&port_num)){
				fill_powerlink_exact_table(dstmac,port_num);
			}
			else{
                fclose(f);
                return -1;
...
char* fn;
void init_complex(){
	set_default_action_gbc_exact();
	set_default_action_ipv6_exact();
    set_default_action_dest_guid_exact();
    set_default_action_powerlink_exact();
