...
void fill_geo_decry_table(uint16_t isvalid[4],uint8_t flag,uint8_t port,uint8_t prefix){
    char buffer[4096]; 
    struct p4_header* h;
    struct p4_add_table_entry* te;
    struct p4_action* a;
    ...
    strcpy(te->table_name, "MyIngress.geo_spd_0");
    ...
    strcpy(a->description.name,"MyIngress.geo_decry");
	...
	while (fgets(line,sizeof(line),f)){
        ...
        if(line[0]=='B'){
            ...
            ,&dummy,&isvalid2[0],&isvalid2[1],&isvalid2[2],&isvalid2[3],&flag,&port,&prefix)){
                fill_geo_decry_table(isvalid2,flag,port,prefix); 
            }
            else{
                fclose(f);
                return -1;
