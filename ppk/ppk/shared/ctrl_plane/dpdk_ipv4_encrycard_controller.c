...
void fill_ipv4_encrypt_table(uint8_t src_ip[4],uint8_t dst_ip[4],uint32_t bitcast,uint32_t sAdrress,uint32_t dAddress,uint8_t flag,uint8_t port,uint8_t prefix){
    char buffer[4096]; 
    struct p4_header* h;
    struct p4_add_table_entry* te;
    struct p4_action* a;
    ...
    strcpy(te->table_name, "MyIngress.ipv4_encrypt_0");
    ...
    strcpy(a->description.name,"MyIngress.ip_sec");
    ...
    strcpy(ap1->name, "sAdrress");
    memcpy(ap1->bitmap, &sAdrress, 4);
    // // ap->bitmap = port;
    // // ap->bitmap[1] = 0;
    ap1->length = 8*4+0;

    ap2 = add_p4_action_parameter(h, a, 4096);
    strcpy(ap2->name, "dAddress");
    memcpy(ap2->bitmap, &dAddress, 4);
    // // ap->bitmap = port;
    // // ap->bitmap[1] = 0;
    ap2->length = 8*4+0;
...
int read_config_from_file(char *filename)
{
	...
	while (fgets(line,sizeof(line),f)){
        ...
        if(line[0]=='B'){
            if(15 == sscanf(line,"%c %hhd.%hhd.%hhd.%hhd %hhd.%hhd.%hhd.%hhd %x %x %x %d %d %d"
            ,&dummy,&ip[0],&ip[1],&ip[2],&ip[3],&ip2[0],&ip2[1],&ip2[2],&ip2[3],&bitcast,&bitcast1,&bitcast2,&flag,&port,&prefix)){
                fill_ipv4_encrypt_table(ip,ip2,bitcast,bitcast1,bitcast2,flag,port,prefix); 
            }
            else{
                fclose(f);
                return -1;
