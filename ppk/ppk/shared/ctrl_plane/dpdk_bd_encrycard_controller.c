...
void fill_gbc_exact_table(uint16_t isvalid[6],uint8_t flag,uint8_t port,uint32_t bitcast){
    ...
    strcpy(te->table_name, "MyIngress.gbc_exact_0");
...
void fill_dest_guid_exact_table(uint8_t dest_guid[4],uint8_t port)
{
    char buffer[2048]; 
    struct p4_header* h;
    struct p4_add_table_entry* te;
...
// void fill_geo_encrypt_table(uint16_t isvalid[4],uint32_t bitcast,uint32_t bitcast1,uint32_t bitcast2,uint8_t flag,uint8_t port,uint8_t prefix){
//     char buffer[4096]; 
//     struct p4_header* h;
//     struct p4_add_table_entry* te;
//     struct p4_action* a;
...
//     strcpy(te->table_name, "MyIngress.geo_encrypt_0");
...
//     strcpy(a->description.name,"MyIngress.geo_mf_sec");
...
//     strcpy(ap1->name, "src_guid");
//     memcpy(ap1->bitmap, &bitcast1, 4);
//     // // ap->bitmap = port;
//     // // ap->bitmap[1] = 0;
//     ap1->length = 8*4+0;

//     ap2 = add_p4_action_parameter(h, a, 4096);
//     strcpy(ap2->name, "dest_guid");
//     memcpy(ap2->bitmap, &bitcast2, 4);
//     // // ap->bitmap = port;
//     // // ap->bitmap[1] = 0;
//     ap2->length = 8*4+0;
...
void fill_geo_encrypt_table(uint16_t isvalid[4],uint32_t bitcast,uint32_t bitcast1,uint32_t bitcast2,uint8_t flag,uint8_t port,uint8_t prefix){
    ...
    a = add_p4_action(h,4096);
    strcpy(a->description.name,"MyIngress.geo_sec");
...
int read_config_from_file(char *filename)
{
	...
	while (fgets(line,sizeof(line),f)){
		...
		if(line[0]=='A'){
            if(10 == sscanf(line,"%c %x %x %x %x %x %x %d %d %x"
            ,&dummy,&isvalid1[0],&isvalid1[1],&isvalid1[2],&isvalid1[3],&isvalid1[4],&isvalid1[5],&flag,&port,&bitcast)){
        ...
        if(line[0]=='B'){
            if(11 == sscanf(line,"%c %x %x %x %x %x %x %x %d %d %d"
            ,&dummy,&isvalid2[0],&isvalid2[1],&isvalid2[2],&isvalid2[3],&bitcast,&bitcast1,&bitcast2,&flag,&port,&prefix)){
                fill_geo_encrypt_table(isvalid2,bitcast,bitcast1,bitcast2,flag,port,prefix); 
            }
            else{
                fclose(f);
                return -1;
        ...
        if(line[0] == 'M'){
			...
			if (6 == sscanf(line,"%c %hhx %hhx %hhx %hhx %hhd",
				&dummy,&dest_guid[0],&dest_guid[1],&dest_guid[2],&dest_guid[3],&port))
			{
				fill_dest_guid_exact_table(dest_guid,port);
			}
			else{
                fclose(f);
                return -1;
