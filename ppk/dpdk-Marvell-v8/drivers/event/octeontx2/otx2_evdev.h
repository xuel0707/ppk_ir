...
#define OTX2_EVENT_PORT(x)  (x>>16)
#define OTX2_EVENT_QUEUE(x)  (x & 0xffff)
...
uint16_t otx2_ssogws_deq_vwqe_ ##name(void *port, struct rte_event *ev,   \
			 uint64_t timeout_ticks);			       \
uint16_t otx2_ssogws_deq_vwqe_burst_ ##name(void *port,		       \
						struct rte_event ev[],	       \
				       uint16_t nb_events,		       \
			 uint64_t timeout_ticks);			       \
...
uint16_t otx2_ssogws_tx_adptr_enq_ ## name(void *port, struct rte_event ev[],\
					   uint16_t nb_events);		     \
uint16_t otx2_ssogws_tx_adptr_vlib_enq_ ## name(void *port,		     \
						    struct rte_event ev[],   \
						    uint16_t nb_events);     \
