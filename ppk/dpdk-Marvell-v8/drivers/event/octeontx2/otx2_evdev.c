...
#undef R
	...
	const event_dequeue_t ssogws_deq_vwqe[2][2][2][2][2][2][2] = {
#define R(name, f6, f5, f4, f3, f2, f1, f0, flags)			\
		[f6][f5][f4][f3][f2][f1][f0] =				\
			otx2_ssogws_deq_vwqe_ ##name,
SSO_RX_ADPTR_ENQ_FASTPATH_FUNC
#undef R
	};

	const event_dequeue_burst_t
		ssogws_deq_vwqe_burst[2][2][2][2][2][2][2] = {
#define R(name, f6, f5, f4, f3, f2, f1, f0, flags)			\
		[f6][f5][f4][f3][f2][f1][f0] =				\
		otx2_ssogws_deq_vwqe_burst_ ##name,
SSO_RX_ADPTR_ENQ_FASTPATH_FUNC
#undef R
	};
...
#undef T
	...
	const event_tx_adapter_enqueue
		ssogws_tx_adptr_vlib_enq[2][2][2][2][2][2][2] = {
#define T(name, f6, f5, f4, f3, f2, f1, f0, sz, flags)			\
		[f6][f5][f4][f3][f2][f1][f0] =				\
			otx2_ssogws_tx_adptr_vlib_enq_ ## name,
SSO_TX_ADPTR_ENQ_FASTPATH_FUNC
#undef T
	};
	...
	if (dev->rx_offloads & NIX_RX_MULTI_SEG_F) {
		...
		if (dev->vwqe_sim_enable) {
			event_dev->dequeue	= ssogws_deq_vwqe
				[!!(dev->rx_offloads &
	...
	if (dev->tx_offloads & NIX_TX_MULTI_SEG_F) {
		...
		if (dev->vwqe_sim_enable) {
			event_dev->txa_enqueue = ssogws_tx_adptr_vlib_enq
			[!!(dev->tx_offloads & NIX_TX_OFFLOAD_SECURITY_F)]
			[!!(dev->tx_offloads & NIX_TX_OFFLOAD_TSO_F)]
			[!!(dev->tx_offloads & NIX_TX_OFFLOAD_TSTAMP_F)]
			[!!(dev->tx_offloads & NIX_TX_OFFLOAD_MBUF_NOFF_F)]
...
#define OTX2_VLIB_ENABLE	"vlib_enable"
#define OTX2_VWQE_SIM_ENABLE	"vwqe_sim_enable"
...
sso_parse_devargs(struct otx2_sso_evdev *dev, struct rte_devargs *devargs)
{
	...
	uint8_t single_ws = 0;
	uint8_t vwqe_sim_enable = 0;
	uint8_t vlib_enable = 0;
	...
	rte_kvargs_process(kvlist, OTX2_SSO_SINGLE_WS, &parse_kvargs_flag,
			   &single_ws);
	rte_kvargs_process(kvlist, OTX2_VWQE_SIM_ENABLE,
			   &parse_kvargs_flag, &vwqe_sim_enable);
	rte_kvargs_process(kvlist, OTX2_VLIB_ENABLE,
			   &parse_kvargs_flag, &vlib_enable);
	rte_kvargs_process(kvlist, OTX2_SSO_GGRP_QOS, &parse_sso_kvargs_dict,
			   dev);

	dev->dual_ws = !single_ws;
	dev->vwqe_sim_enable = vwqe_sim_enable;
	dev->vlib_enable = vlib_enable;
