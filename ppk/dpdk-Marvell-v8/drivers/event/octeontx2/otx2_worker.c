...
{									\
	...
	return otx2_ssogws_get_work(ws, ev, flags, ws->lookup_mem);	\
									...
									\
uint16_t __hot								\
otx2_ssogws_deq_vwqe_ ##name(void *port, struct rte_event *ev,	\
			uint64_t timeout_ticks)				\
{									\
	struct otx2_ssogws *ws = port;					\
									\
	...
	if (ws->swtag_req) {						\
		...
		return 1;						\
									...
									\
	return otx2_ssogws_get_work_vwqe(ws, ev, flags, ws->lookup_mem);\
}									\
									\
uint16_t __hot								\
otx2_ssogws_deq_vwqe_burst_ ##name(void *port,			\
			struct rte_event ev[],				\
			uint16_t nb_events,				\
			uint64_t timeout_ticks)				\
{									\
	RTE_SET_USED(nb_events);					\
									\
	return otx2_ssogws_deq_vwqe_ ##name(port, ev,		\
	timeout_ticks);							\
}									\
									\
uint16_t __hot								\
...
#define T(name, f6, f5, f4, f3, f2, f1, f0, sz, flags)			\
uint16_t __hot								\
otx2_ssogws_tx_adptr_vlib_enq_ ## name(void *port, struct rte_event ev[],\
				  uint16_t nb_events)			\
{									\
	struct otx2_ssogws *ws = port;					\
	uint64_t cmd[sz];						\
									\
	return otx2_ssogws_event_tx_vlib(ws, ev, cmd, flags, nb_events);\
}
SSO_TX_ADPTR_ENQ_FASTPATH_FUNC
#undef T
