...
static inline void
otx2_nix_xmit_prepare_from_vlib(struct rte_mbuf *m, uint64_t *cmd,
			const uint16_t flags, dpdk_otx2_per_thread_data_t *ptd)
{
	struct nix_send_ext_s *send_hdr_ext;
	struct nix_send_hdr_s *send_hdr;
	uint64_t ol_flags = 0;
	union nix_send_hdr_w1_u w1;
	union nix_send_sg_s *sg;
	dpdk_vlib_buffer_t *vbuf = (dpdk_vlib_buffer_t *)m;
	...
	if (!(flags & NIX_TX_MULTI_SEG_F)) {
		send_hdr->w0.total = vbuf->current_length;
		mp = ptd->otx2_mempool_by_index[vbuf->buffer_pool_index];
		send_hdr->w0.aura =  npa_lf_aura_handle_to_aura(mp->pool_id);
	}
	...
	if (flags & NIX_TX_OFFLOAD_L3_L4_CSUM_F) {
		/* Inner L3 */
		w1.ol3ptr = (uint8_t)(dpdk_vnet_buffer(vbuf)->l3_hdr_offset);
		w1.ol4ptr = (uint8_t)(dpdk_vnet_buffer(vbuf)->l4_hdr_offset);
		/* Increment it by 1 if it is IPV4 as 3 is with csum */
		w1.ol3type = ((!!(ol_flags & PKT_TX_IPV4)) << 1) +
			((!!(ol_flags & PKT_TX_IPV6)) << 2) +
			!!(ol_flags & PKT_TX_IP_CKSUM);
	...
	if (!(flags & NIX_TX_MULTI_SEG_F)) {
		sg->seg1_size = vbuf->current_length;
		*(rte_iova_t *)(++sg) = (rte_iova_t)
					(vbuf->data + vbuf->current_data);
