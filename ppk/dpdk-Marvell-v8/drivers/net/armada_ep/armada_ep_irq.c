			      ...
			      uint16_t rx_queue_id)
{
	...
	ret = armada_ep_irq_config(intr_handle, q->intr_vec,
				   VFIO_IRQ_SET_DATA_EVENTFD);
	if (ret)
		ARMADA_EP_LOG(ERR, "Error during VFIO interrupt enable, "
			      "ret = %d", ret);
...
int
armada_ep_rx_queue_intr_disable(struct rte_eth_dev *eth_dev __rte_unused,
			       uint16_t rx_queue_id __rte_unused)
{
	...
	if (ret)
		ARMADA_EP_LOG(ERR, "Error during VFIO interrupt disable, "
			      "ret = %d", ret);
