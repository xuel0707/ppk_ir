...
armada_ep_inq_desc_to_ol_flags(struct armada_ep_desc *desc)
{
	...
	l4_status = ((desc->cmds[0] & ARMADA_EP_RXD_L4_STATUS_MASK) >> 13);
	...
	ipv4_status = ((desc->cmds[0] & ARMADA_EP_RXD_IPV4_STATUS_MASK) >> 23);
...
armada_ep_tx_pkt_burst(void *txq, struct rte_mbuf **tx_pkts, uint16_t nb_pkts)
{
	...
	shadow_txq = &sw_txq->shadow_txqs[core_id];
...
armada_ep_dev_close(struct rte_eth_dev *dev)
{
	...
	for (i = 0; i < priv->num_tx_queues; i++) {
		...
		for (j = 0; j < RTE_MAX_LCORE; j++) {
			rte_free(sw_txq->shadow_txqs[j].ent);
			rte_free(sw_txq->shadow_txqs[j].descs);
		}
...
err_sw_tx_queue_setup:
	if (sw_txq) {
		for (i = 0; i < RTE_MAX_LCORE; i++) {
			rte_free(sw_txq->shadow_txqs[i].ent);
			rte_free(sw_txq->shadow_txqs[i].descs);
		}
	}
	rte_free(sw_txq);
