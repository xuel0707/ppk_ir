...
const bit<4> TYPE_geo_tsb_s = 0xC; 
...
control MyIngress(inout headers hdr,
    ...
    action geo_sec(bit<32> spi, bit<32> PosLat, bit<32> PosLon, bit<8> name1, bit<8>name2, bit<8>name3) {
        
        encrypt_with_payload(spi, name1, name2, name3);
        hdr.ethernet.ether_type=0x8947;
        hdr.geo.nh_common=0x000B;
        hdr.gbc.geoAreaPosLat=PosLat;
        hdr.gbc.geoAreaPosLon=PosLon;
        hdr.geo.pl= (bit<16>)standard_metadata.packet_length - 70;
    }

    action geo_mf_sec(bit<32> spi, bit<32> src_guid, bit<32> dest_guid, bit<8> name1, bit<8>name2, bit<8>name3) {
        
        encrypt_with_payload(spi, name1, name2, name3);
        hdr.ethernet.ether_type=0x27c0;
        hdr.geo.setInvalid();
        hdr.gbc.setInvalid();
        hdr.mf.setValid();
        hdr.mf.mf_type = 0x000A;
