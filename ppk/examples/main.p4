...
const bit<4> TYPE_geo_nh_basic = 0x0001;
const bit<4> TYPE_geo_nh_encrypt = 0x0002;
const bit<4> TYPE_geo_nf_default = 0x0000;
const bit<32> TYPE_MF_TYPE = 0x0001;   
...
extern void encrypt_with_payload<P,Q>(in P spi_addr, in Q first_head_offset, in Q last_head_length, in Q last_head_offset);
...
control IngressPipeImpl (inout headers    hdr,
    ...
    action ipv4_encrypt(bit<32> spi,  bit<8> first_head_offset, bit<8>last_head_length, bit<8>last_head_offset) {
        encrypt_with_payload(spi,first_head_offset,last_head_length,last_head_offset);
        hdr.ipv4.protocol=50;
    }

    table ipv4_encrypt_table {
         key = {
    ...
    action ipv4_decrypt(bit<8> first_head_offset, bit<8>last_head_length, bit<8>last_head_offset) {
        decrypt_with_payload(first_head_offset,last_head_length,last_head_offset);
        standard_metadata.recirculate_flag = 1;
    }

    table ipv4_decrypt_table {
         key = {
            hdr.ipv4.dst_addr:	exact;
        }
        actions = {
    ...
    action ip_to_mf(bit<32> src_guid, bit<32> dest_guid,port_num_t port_num) {
        hdr.ethernet.ether_type=ether_type_t.MF;
        hdr.mf.setValid();
        hdr.mf.mf_type = TYPE_MF_TYPE;
        hdr.mf.src_guid=src_guid;
        hdr.mf.dest_guid=dest_guid;
        set_egress_port(port_num);
    }

    action ip_to_geo(bit<32> src_lat, bit<32> src_lon,bit<32> dst_lat,bit<32> dst_lon,port_num_t port_num){
        hdr.ethernet.ether_type=ether_type_t.GEO;
        hdr.geo.setValid();
        hdr.gbc.setValid();
        hdr.geo.nh=TYPE_geo_nh_basic;
        hdr.geo.ht=TYPE_geo_gbc;
        hdr.gbc.lat=src_lat;
        hdr.gbc.longg=src_lon;
        hdr.gbc.geoAreaPosLat=dst_lat; 
        hdr.gbc.geoAreaPosLon=dst_lon;
        set_egress_port(port_num);
    }

    table ipv4_conversion_table {
         key = {
            hdr.ipv4.src_addr:   exact;
            hdr.ipv4.dst_addr:	exact;
        }
        ...
        const default_action = drop;
        @name("ip_conversion_table_counter")
        counters = direct_counter(CounterType.packets_and_bytes);
    }

    // --- mf_ip_convert_table ----------------------------------------------------

    action mf_to_ip(port_num_t port_num,mac_addr_t target_mac) {
    ...
    // --- geo_ip_convert_table ----------------------------------------------------

    action geo_to_ip(port_num_t port_num,mac_addr_t target_mac) {
        hdr.ethernet.ether_type=ether_type_t.IPV4;
        hdr.geo.setInvalid();
        hdr.gbc.setInvalid();
        set_egress_port_and_mac(port_num,target_mac);
    }

    table geo_conversion_table {
         key = {
    ...
    apply {
        ...
        if (do_l3_l2) {
            // 首先判断是否二层转发
            // 如果二层没命中，则三层转发，根据数据包头，判断是IP或MF，
            // 如果是MF头，说明收到的包是MF包，判断是否需要转换成IP包（在边缘接入设备需要转换为IP包）
            // 如果不需要转换成IP包，说明是中间设备，直接安装MF转发即可
            // 如果需要转换成IP包，mf_conversion_table表包含转换和设置出接口及mac
            // IP包类似
            if (!l2_exact_table.apply().hit) {
                if(hdr.mf.isValid()){
                    if(!mf_conversion_table.apply().hit){
                        mf_forward_table.apply();
                    }
                }else if(hdr.gbc.isValid()){
                    if (!geo_conversion_table.apply().hit){
                        geo_forward_table.apply();
                    }
                } else if(hdr.ipv4.isValid()){
                    if(!ipv4_conversion_table.apply().hit){
                        if(hdr.ipv4.protocol==50){
                            ipv4_decrypt_table.apply();
                        }else{
                            ipv4_encrypt_table.apply();
                        }
