...
control IngressPipeImpl (inout headers    hdr,
    ...
    action ip_encrypt() {
        encrypt_with_payload((bit<32>)0, (bit<8>)14, (bit<8>)20, (bit<8>)14);
        hdr.ipv4.protocol=50;
    }

    action ip_decrypt() {
        decrypt_with_payload((bit<8>)14,(bit<8>)20,(bit<8>)14);
    ...
    apply {
        ...
        if (do_l3_l2) {
            if (!l2_exact_table.apply().hit) {
                if(hdr.mf.isValid()){
                    if(mf_conversion_table.apply().miss){
                        mf_forward_table.apply();
                        
                    }
                }
                if(hdr.ipv4.isValid()){
                    if(ipv4_conversion_table.apply().miss){
