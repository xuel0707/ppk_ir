...
control MyIngress(inout headers hdr,
	...
	direct_counter(CounterType.packets_and_bytes) guid_exact_counter;
    ...
    apply {
	    if (standard_metadata.ingress_port == CPU_PORT) {
            ...
            if (standard_metadata.egress_spec < MAX_PORTS) {
                tx_port_counter.count((bit<32>) standard_metadata.egress_spec+1);
            }
            if (standard_metadata.ingress_port < MAX_PORTS) {
                rx_port_counter.count((bit<32>) standard_metadata.ingress_port+1);
            }
		}
