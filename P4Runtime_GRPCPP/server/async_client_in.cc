...
public:
	...
	explicit AbstractAsyncClientCall(CompletionQueue& cq, std::unique_ptr<P4Runtime::Stub>& stub):
        callStatus(PROCESS),
        cq_(cq),
        stub_(stub)
        {}
	...
	void printReply()
	{
		...
		const uint8_t *p = (uint8_t*)reply_data.data();

		for (size_t i = 0; i < reply_data.length(); i++)
        {
        printf("%02x",*p);
            p++;
        }
...
class AsyncClientCallMM : public AbstractAsyncClientCall
{
	...
	bool writing_mode_;
public:
	AsyncClientCallMM(CompletionQueue& cq, std::unique_ptr<P4Runtime::Stub>& stub):
	AbstractAsyncClientCall(cq, stub), mcounter(0), writing_mode_(false)
	{
		// std::cout << "[Client]: Create new Call" << std::endl;
	...
	virtual void Proceed(bool ok = true) override
	{
		...
		{
			// std::cout << "[Client]: callStatus == FINISH\n" << std::endl;
...
{
	P4RuntimeClient client(grpc::CreateChannel("192.168.133.202:50052", grpc::InsecureChannelCredentials()));
