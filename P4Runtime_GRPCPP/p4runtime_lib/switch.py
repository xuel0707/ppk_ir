...
class SwitchConnection(object):
    ...
    def __init__(self, name=None, address='127.0.0.1:50051', device_id=0,
        ...
        self.packet = str(Ether(dst='ff:ff:ff:ff:ff:ff')/IP(dst='127.0.0.1')/TCP(dport=1)/'woshidashuaibi\n')
    ...
    def Packet_Out(self, dry_run=False, **kwargs):
        request = p4runtime_pb2.StreamMessageRequest()
        request.packet.payload = self.packet
        metada=request.packet.metadata.add()
        metada.metadata_id= 1
        metada.value = "11111"
        #request.packet.metadata.value = 111

        if dry_run:
            print "P4Runtime packet out: ", request
        else:
            self.requests_stream.put(request)
            for item in self.stream_msg_resp:
                return item 
    ...
    def SetForwardingPipelineConfig(self, p4info, dry_run=False, **kwargs):
        ...
        if dry_run:
            print "P4Runtime SetForwardingPipelineConfig:00000", request
        else:
...
###############################################################################################################
    ...
    def ReadRegisterEntry(self, register_entry, dry_run=False):
	request = p4runtime_pb2.ReadRequest()
        request.device_id = self.device_id
        entity = request.entities.add()
        entity.register_entry.CopyFrom(register_entry)
	self.client_stub.Read(request)
###############################################################################################################
    def WriteTableEntry(self, table_entry, dry_run=False):
	print("writeeeeeeeeeeeeeee")
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = 1
        update = request.updates.add()
    ...
    def Write2TableEntry(self, table_entry, dry_run=False):
	request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = 1
        update = request.updates.add()
    ...
    def Read2TableEntries(self, table_entry, dry_run=False):
	print("readtableentryyyyyyyyyyyyyy2222222")
        request = p4runtime_pb2.ReadRequest()
        request.device_id = self.device_id
        entity = request.entities.add()
        table_entry = entity.table_entry
 ...
 #    def StreamChannelTableEntry(self):
	# request = p4runtime_pb2.StreamMessageRequest()
	# request.packet.payload = "iampayload"
	# #request.packet.metadata = 2
	# #request.packet.metadata.value = "medata_value"
    ...
    def WriteMulticastGroupEntry(self, mc_entry, dry_run=False):
        ...
        if dry_run:
            print "P4Runtime Write:3333", request
        else:
            self.client_stub.Write(request)
