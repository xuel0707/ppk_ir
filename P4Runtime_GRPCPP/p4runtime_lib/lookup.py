...
def program_switch(addr, device_id, sw_conf_file, workdir, proto_dump_fpath):
	...
	try:
		...
		if 'register_entries' in sw_conf:
			register_entries = sw_conf['register_entries']
			print(register_entries)
			info("Inserting %d register entries..." % len(register_entries))
			for entry in register_entries:
				lookupRegisterEntry(sw,entry,p4info_helper)
				

	finally:
		sw.shutdown()

def lookupRegisterEntry(sw,entry,p4info_helper):
    index = 0
    register_name = entry['register']
    register_entry = p4info_helper.buildRegisterEntry(register_name,index,data=None)
    sw.ReadRegisterEntry(register_entry)
def insertRegisterEntry(sw,entry,p4info_helper):
    index = 0
    data = 1
    register_name = entry['register']
    register_entry = p4info_helper.buildRegisterEntry(register_name,index,data)
    print(dir(sw))
    sw.WriteRegisterEntry(register_entry)
...
def lookupTableEntry(sw, flow, p4info_helper):
	...
	sw.ReadTableEntries(table_entry.table_id)
