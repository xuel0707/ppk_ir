...
class RuntimeAPI(cmd.Cmd):
    ...
    def do_table_insert(self,line):
        ...
        with open(table_entry_file,'r') as f:
            ...
            TABLES[json.dumps(sw_conf['table_entries']['table'])] = json.dumps(sw_conf['table_entries'])
            ACTIONS[json.dumps(sw_conf['table_entries']['action'])] = json.dumps(sw_conf['table_entries']['action_params'])
    ...
    def do_table_delete(self,line):
        ...
        with open(poto_file,'r') as f:
            sw_conf = json_load_byteified(f)
            del TABLES[json.dumps(sw_conf['table_entries']['table'])]
...
def lookupTableEntry():
	...
	sw.ReadTableEntries(table_entry)
