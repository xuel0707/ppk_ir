...
#else /* RTE_VERSION >= RTE_VERSION_NUM(18, 5, 0, 0) */
static int
eth_txgbe_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
		struct rte_pci_device *pci_dev)
{
...
#if RTE_VERSION_NUM(17, 11, 0, 0) <= RTE_VERSION && RTE_VERSION < RTE_VERSION_NUM(17, 11, 255, 255)
static void
txgbe_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue, int on)
{
	...
	wr32(hw, TXGBE_RXCFG(rxq->reg_idx), 0);
	msec_delay(100);
	txgbe_dev_store_rx_queue(hw, rxq->reg_idx);

	wr32m(hw, TXGBE_RXCFG(rxq->reg_idx),
		TXGBE_RXCFG_VLAN | TXGBE_RXCFG_ENA, rxcfg);
}
#else
static void
txgbe_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue, int on)
