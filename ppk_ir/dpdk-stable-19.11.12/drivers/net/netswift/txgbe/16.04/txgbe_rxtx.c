		...
		__rte_unused uint64_t *mdata)
{
	...
	if (ol_flags & PKT_TX_TCP_SEG) {
		...
		case PKT_TX_UDP_CKSUM:
			mss_l4len_idx |= TXGBE_TXD_L4LEN(sizeof(struct udp_hdr));
			tx_offload_mask.l2_len |= ~0;
			tx_offload_mask.l3_len |= ~0;
			break;
		case PKT_TX_TCP_CKSUM:
			mss_l4len_idx |= TXGBE_TXD_L4LEN(sizeof(struct tcp_hdr));
			tx_offload_mask.l2_len |= ~0;
			tx_offload_mask.l3_len |= ~0;
			break;
		case PKT_TX_SCTP_CKSUM:
			mss_l4len_idx |= TXGBE_TXD_L4LEN(sizeof(struct sctp_hdr));
			tx_offload_mask.l2_len |= ~0;
			tx_offload_mask.l3_len |= ~0;
			break;
		default:
...
txgbe_parse_tun_ptid(struct rte_mbuf *tx_pkt)
{
	...
	l2_mac = l2_none + sizeof(struct ether_hdr);
	l2_mac_vlan = l2_mac + sizeof(struct vlan_hdr);
		...
		uint16_t nb_pkts)
{
	...
	volatile struct txgbe_tx_desc *txd; //, *txp;
...
rx_desc_error_to_pkt_flags(uint32_t rx_status)
{
	...
	//};
	//pkt_flags = error_to_pkt_flags_map[TXGBE_RXD_ERR_CSUM(rx_status)];
		    ...
		    bool bulk_alloc)
{
	...
	while (nb_rx < nb_pkts) {
		...
		struct txgbe_scattered_rx_entry *next_sc_entry;
...
static uint64_t map_tx_flags_to_offloads(uint32_t txq_flags)
{
	uint64_t offloads = 0;
			 ...
			 const struct rte_eth_txconf *tx_conf)
{
	...
	offloads = map_tx_flags_to_offloads(tx_conf->txq_flags);
...
void txgbe_configure_port(struct rte_eth_dev *dev)
{
	...
	wr32(hw, TXGBE_EXTAG,
		TXGBE_EXTAG_ETAG(ETHER_TYPE_ETAG) |
		TXGBE_EXTAG_VLAN(ETHER_TYPE_QINQ));

	/* default inner vlan tpid */
	wr32m(hw, TXGBE_VLANCTL,
		TXGBE_VLANCTL_TPID_MASK,
		TXGBE_VLANCTL_TPID(ETHER_TYPE_VLAN));
	wr32m(hw, TXGBE_DMATXCTRL,
		TXGBE_DMATXCTRL_TPID_MASK,
		TXGBE_DMATXCTRL_TPID(ETHER_TYPE_VLAN));
...
/**
 * txgbe_get_rscctl_maxdesc - Calculate the RSCCTL[n].MAXDESC for PF
 *
...
txgbe_set_rx_function(struct rte_eth_dev *dev)
{
	...
	if (txgbe_rx_vec_dev_conf_condition_check(dev) ||
		...
		PMD_INIT_LOG(DEBUG, "Port[%d] doesn't meet Vector Rx "
				    "preconditions or RTE_TXGBE_INC_VECTOR is "
				    "not enabled",
			     dev->data->port_id);
...
txgbe_set_rsc(struct rte_eth_dev *dev)
{
	...
	for (i = 0; i < dev->data->nb_rx_queues; i++) {
		 ...
		 *
		 * if RSC is enabled the SRRCTL[n].BSIZEHEADER
		...
		if (1) //fixme: 10G and 1G
			eitr |= TXGBE_ITR_IVAL_10G(
					TXGBE_QUEUE_ITR_INTERVAL_DEFAULT);
		else
			eitr |= TXGBE_ITR_IVAL_1G(
					TXGBE_QUEUE_ITR_INTERVAL_DEFAULT);
...
txgbe_dev_rx_init(struct rte_eth_dev *dev)
{
	...
	if (rx_conf->jumbo_frame == 1) {
		//wr32m(hw, TXGBE_MACRXCFG, TXGBE_MACRXCFG_JUMBO,
		//	TXGBE_MACRXCFG_JUMBO);
		wr32m(hw, TXGBE_FRMSZ, TXGBE_FRMSZ_MAX_MASK,
			TXGBE_FRMSZ_MAX(rx_conf->max_rx_pkt_len));
	} else {
		//wr32m(hw, TXGBE_MACRXCFG, TXGBE_MACRXCFG_JUMBO, 0);
		wr32m(hw, TXGBE_FRMSZ, TXGBE_FRMSZ_MAX_MASK,
...
txgbe_setup_loopback_link_raptor(struct txgbe_hw *hw)
{
	...
	//	if (hw->mac.acquire_swfw_sync(hw, TXGBE_MNGSEM_SWPHY) !=
	//			0) {
	...
	//		TXGBE_AUTOC_LMS_10G_LINK_NO_AN | TXGBE_AUTOC_FLU);
...
txgbe_dev_save_rx_queue(struct txgbe_hw *hw, uint16_t rx_queue_id)
{
	...
	*(reg++) = rd32(hw, TXGBE_RXBAH(rx_queue_id));
	//*(reg++) = rd32(hw, TXGBE_RXRP(rx_queue_id));
	//*(reg++) = rd32(hw, TXGBE_RXWP(rx_queue_id));
...
txgbe_dev_store_rx_queue(struct txgbe_hw *hw, uint16_t rx_queue_id)
{
	...
	//wr32(hw, TXGBE_RXRP(rx_queue_id), *(reg++));
	//wr32(hw, TXGBE_RXWP(rx_queue_id), *(reg++));
...
txgbe_dev_save_tx_queue(struct txgbe_hw *hw, uint16_t tx_queue_id)
{
	...
	*(reg++) = rd32(hw, TXGBE_TXBAH(tx_queue_id));
	//*(reg++) = rd32(hw, TXGBE_TXRP(tx_queue_id));
	//*(reg++) = rd32(hw, TXGBE_TXWP(tx_queue_id));
...
txgbe_dev_store_tx_queue(struct txgbe_hw *hw, uint16_t tx_queue_id)
{
	...
	//wr32(hw, TXGBE_TXRP(tx_queue_id), *(reg++));
	//wr32(hw, TXGBE_TXWP(tx_queue_id), *(reg++));
...
/* Stubs needed for linkage when CONFIG_RTE_TXGBE_INC_VECTOR is set to 'n' */
__rte_weak int
txgbe_rx_vec_dev_conf_condition_check(struct rte_eth_dev __rte_unused *dev)
{
	return -1;
}
...
__rte_weak int
txgbe_rxq_vec_setup(struct txgbe_rx_queue __rte_unused *rxq)
{
	return -1;
}
