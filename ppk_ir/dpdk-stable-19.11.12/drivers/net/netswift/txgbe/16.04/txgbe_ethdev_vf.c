...
static void txgbevf_dev_info_get(struct rte_eth_dev *dev,
...
static void txgbevf_dev_stats_reset(struct rte_eth_dev *dev);
...
static void txgbevf_dev_promiscuous_enable(struct rte_eth_dev *dev);
static void txgbevf_dev_promiscuous_disable(struct rte_eth_dev *dev);
...
#else
static void txgbevf_dev_interrupt_handler(struct rte_intr_handle *handle,
				void *param);
#endif
...
eth_txgbevf_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	eth_dev->data->mac_addrs = rte_zmalloc("txgbevf", ETHER_ADDR_LEN *
...
static struct eth_driver rte_txgbevf_pmd = {
	.pci_drv = {
		.id_table = pci_id_txgbevf_map,
		.drv_flags = RTE_PCI_DRV_NEED_MAPPING,
#if RTE_VERSION < RTE_VERSION_NUM(16, 11, 0, 0)
		.name = "rte_txgbevf_pmd",
#else
		.probe = rte_eth_dev_pci_probe,
		.remove = rte_eth_dev_pci_remove,
#endif
	},
	.eth_dev_init = eth_txgbevf_dev_init,
	.eth_dev_uninit = eth_txgbevf_dev_uninit,
 ...
 */
static int
rte_txgbevf_pmd_init(const char *name __rte_unused, const char *param __rte_unused)
{
	PMD_INIT_FUNC_TRACE();

	rte_eth_driver_register(&rte_txgbevf_pmd);
	return 0;
}

static struct rte_driver rte_txgbevf_driver = {
	.type = PMD_PDEV,
	.init = rte_txgbevf_pmd_init,
};
...
txgbevf_intr_enable(struct rte_eth_dev *dev)
{
	...
	/* Save TXGBE_VTEIMS value to mask. */
	intr->mask_misc = 0;
}
		     ...
		     uint8_t queue, uint8_t msix_vector)
{
	...
	if (direction == -1) {
		...
		/* rx or tx cause */
		//msix_vector |= TXGBE_VFIVAR_VLD; /* Workround for ICR lost */
...
#if RTE_VERSION >= RTE_VERSION_NUM(17, 5, 0, 0)
static int
txgbevf_add_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr,
...
static void
txgbevf_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *addr)
{
	struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
...
txgbevf_dev_promiscuous_enable(struct rte_eth_dev *dev)
{
	...
	hw->mac.update_xcast_mode(hw, TXGBEVF_XCAST_MODE_PROMISC);
}
...
txgbevf_dev_promiscuous_disable(struct rte_eth_dev *dev)
{
	...
	hw->mac.update_xcast_mode(hw, TXGBEVF_XCAST_MODE_NONE);
}
...
txgbevf_dev_allmulticast_enable(struct rte_eth_dev *dev)
{
	...
	hw->mac.update_xcast_mode(hw, TXGBEVF_XCAST_MODE_ALLMULTI);
}
...
txgbevf_dev_allmulticast_disable(struct rte_eth_dev *dev)
{
	...
	hw->mac.update_xcast_mode(hw, TXGBEVF_XCAST_MODE_MULTI);
}
...
#else /* RTE_VERSION < RTE_VERSION_NUM(17, 5, 0, 0) */
static void
txgbevf_dev_interrupt_handler(__rte_unused struct rte_intr_handle *handle,
			      void *param)
{
	struct rte_eth_dev *dev = (struct rte_eth_dev *)param;
...
#if RTE_VERSION < RTE_VERSION_NUM(16, 7, 0, 0)
	PMD_REGISTER_DRIVER(rte_txgbevf_driver);
#elif RTE_VERSION < RTE_VERSION_NUM(16, 11, 0, 0)
	PMD_REGISTER_DRIVER(rte_txgbevf_driver, txgbevf);
	DRIVER_REGISTER_PCI_TABLE(txgbevf, pci_id_txgbevf_map);
#elif RTE_VERSION < RTE_VERSION_NUM(17, 2, 0, 0)
	RTE_PMD_REGISTER_PCI(net_txgbe_vf, rte_txgbevf_pmd.pci_drv);
