...
/**
 *  txgbe_read_phy_if - Read NW_MNG_IF_SEL register
 *  @hw: pointer to hardware structure
 *
...
static s32 txgbe_read_phy_if(struct txgbe_hw *hw)
{
	...
	//	hw->phy.nw_mng_if_sel |= TXGBE_ETHPHYIF_IO_EPHY;
	//}
					   ...
					   u32 *firmware_version)
{
	...
	*firmware_version = (eeprom_verh << 16) | eeprom_verl;
...
txgbe_set_link_to_kx4(struct txgbe_hw *hw, bool autoneg)
{
	...
	if (hw->link_status == TXGBE_LINK_STATUS_KX4) {
		//printk("TXGBE_HW link status = %d, skip config kx4\n", hw->link_status);
		goto out;
	}
	...
	wr32_epcs(hw, TXGBE_PHY_VCO_CAL_LD0, TXGBE_PHY_VCO_CAL_LD0_OTHER);

	//Set VR XS, PMA, or MII Synopsys Enterprise Gen5 12G PHY VCO
	//Calibration Load 1 Register  Bit[12:0] = 13'd1360  //VCO_LD_VAL_1
	wr32_epcs(hw, TXGBE_PHY_VCO_CAL_LD1, TXGBE_PHY_VCO_CAL_LD0_OTHER);

	//Set VR XS, PMA, or MII Synopsys Enterprise Gen5 12G PHY VCO
	//Calibration Load 2 Register  Bit[12:0] = 13'd1360  //VCO_LD_VAL_2
	wr32_epcs(hw, TXGBE_PHY_VCO_CAL_LD2, TXGBE_PHY_VCO_CAL_LD0_OTHER);
	//Set VR XS, PMA, or MII Synopsys Enterprise Gen5 12G PHY VCO
	//Calibration Load 3 Register  Bit[12:0] = 13'd1360  //VCO_LD_VAL_3
	wr32_epcs(hw, TXGBE_PHY_VCO_CAL_LD3, TXGBE_PHY_VCO_CAL_LD0_OTHER);
	//Set VR XS, PMA, or MII Synopsys Enterprise Gen5 12G PHY VCO
	...
	wr32_epcs(hw, TXGBE_PHY_MPLLA_CTL2, TXGBE_PHY_MPLLA_CTL2_DIV_CLK_EN_10);

	wr32_epcs(hw, 0x1f0000, 0x0);
	wr32_epcs(hw, 0x1f8001, 0x0);
	wr32_epcs(hw, SR_MII_MMD_DIGI_CTL, 0x0);

	if (TXGBE_KX_TXRX_PIN == 1)
		wr32_epcs(hw, VR_XS_OR_PCS_MMD_DIGI_CTL2, 0xff);
			       ...
			       bool autoneg)
{
	...
	if (hw->link_status == TXGBE_LINK_STATUS_KX) {
		//printk("TXGBE_HW link status = %d, skip configure kx\n", hw->link_status);
		goto out;
	}
...
void txgbe_autoc_write(struct txgbe_hw *hw, u64 autoc)
{
	...
	wr32m(hw, TXGBE_MACTXCFG, TXGBE_MACTXCFG_SPEED_MASK | TXGBE_MACTXCFG_TE,
		mactxcfg | TXGBE_MACTXCFG_TE);
}
...
s32 txgbe_handle_bp_flow(u32 link_mode, struct txgbe_hw *hw)
{
	...
	/* -1 : none link mode matched, exit
         ...
         *  1 : current link mode not matched, set to matched link mode,
         *      re-start AN73 and return
	 */
	if (status == 1) {
		txgbe_clear_bp_intr(0, 2, hw);

		/* Restart the AN73 by setting Bit 9 of the SR_AN_CTRL register */
		value = rd32_epcs(hw, SR_AN_CTRL);
		value |= SR_AN_CTRL_AN_EN | SR_AN_CTRL_RSTRT_AN;
		wr32_epcs(hw, SR_AN_CTRL, value);
		value = rd32_epcs(hw, SR_AN_CTRL);
		BP_LOG("Restart the Backplane AN73: 0x%x\n", value);
	} else if (status == -1) {
		return status;
	}

	/* Wait all the AN73 pages exchanging to be done */
	if (local_ability.next_page || lp_ability.next_page) {
		BP_LOG("Wait all the AN73 pages exchanging to be done.\n");
		for (i = 0; i < 20; i++) {			
			value = rd32_epcs(hw, VR_AN_INTR);
			BP_LOG("Read VR AN MMD Interrupt Register: 0x%x\n", value);
			if (value & VR_AN_INTR_PG_RCV) {
				/* Clear the AN_PG_RCV interrupt */
				txgbe_clear_bp_intr(2, 0, hw);

				/* Get the link partner AN73 Next Page Ability */
				txgbe_get_bp_ability(&lp_ability, 2, hw);
				if (lp_ability.next_page == 0)
					break;
			}
			msec_delay(200);
		}
		...
		if (i == 20)
			BP_LOG("ERROR: Wait all the AN73 next pages to be exchanged Timeout.\n");
	}
#endif
	wr32_epcs(hw, SR_AN_CTRL, 0);
	wr32_epcs(hw, VR_AN_KR_MODE_CL, 0);
	...
	if ((local_ability.fec_ability & lp_ability.fec_ability) == 0x03) {
		...
		wr32_epcs(hw, SR_PMA_KR_FEC_CTRL,
					SR_PMA_KR_FEC_CTRL_EN);
	} else {
