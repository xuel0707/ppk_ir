...
static int txgbe_add_rar(struct rte_eth_dev *dev, struct ether_addr *mac_addr,
...
int txgbe_logtype_rx = RTE_LOGTYPE_PMD;
int txgbe_logtype_tx = RTE_LOGTYPE_PMD;
int txgbe_logtype_tx_free = RTE_LOGTYPE_PMD;
...
RTE_INIT(txgbe_init_log)
{
	...
	txgbe_logtype_rx = rte_log_register("pmd.net.txgbe.rx");
	if (txgbe_logtype_rx >= 0)
		rte_log_set_level(txgbe_logtype_rx, RTE_LOG_DEBUG);
#endif

#ifdef RTE_LIBRTE_TXGBE_DEBUG_TX
	txgbe_logtype_tx = rte_log_register("pmd.net.txgbe.tx");
	if (txgbe_logtype_tx >= 0)
		rte_log_set_level(txgbe_logtype_tx, RTE_LOG_DEBUG);
#endif

#ifdef RTE_LIBRTE_TXGBE_DEBUG_TX_FREE
	txgbe_logtype_tx_free = rte_log_register("pmd.net.txgbe.tx_free");
	if (txgbe_logtype_tx_free >= 0)
		rte_log_set_level(txgbe_logtype_tx_free, RTE_LOG_DEBUG);
#endif
}
