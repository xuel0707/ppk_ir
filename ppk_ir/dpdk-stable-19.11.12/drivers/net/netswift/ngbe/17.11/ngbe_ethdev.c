...
static int ngbe_dev_stats_get(struct rte_eth_dev *dev,
...
static int ngbe_vlan_offload_set(struct rte_eth_dev *dev, int mask);
		...
		struct rte_pci_device *pci_dev)
{
	return rte_eth_dev_pci_generic_probe(pci_dev,
		sizeof(struct ngbe_adapter), eth_ngbe_dev_init);
}
...
ngbe_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue, int on)
{
	...
	wr32(hw, NGBE_RXCFG(rxq->reg_idx), 0);
	msec_delay(100);
	ngbe_dev_store_rx_queue(hw, rxq->reg_idx);

	wr32m(hw, NGBE_RXCFG(rxq->reg_idx),
		NGBE_RXCFG_VLAN | NGBE_RXCFG_ENA, rxcfg);
}
...
ngbe_vlan_hw_strip_config(struct rte_eth_dev *dev)
{
	...
	for (i = 0; i < dev->data->nb_rx_queues; i++) {
		...
		if (rxq->offloads & DEV_RX_OFFLOAD_VLAN_STRIP) {
			ngbe_vlan_strip_queue_set(dev, i, 1);
		} else {
			ngbe_vlan_strip_queue_set(dev, i, 0);
		}
	}
}
