...
desc_to_olflags_v_ipsec(__m128i descs[4], struct rte_mbuf **rx_pkts)
{
	...
	const __m128i ipsec_sterr_msk =
			_mm_set1_epi32(NGBE_RXD_STAT_SECP |
				       NGBE_RXD_ERR_SECERR);
	...
	struct rte_mbuf **rx_pkts)
{
	...
	const __m128i vlan_csum_msk = _mm_set_epi16(
		...
		NGBE_RXD_STAT_VLAN, NGBE_RXD_STAT_VLAN);
	//fixme: need to check both NGBE_RXD_STAT_VLAN and PKT_RX_VLAN_STRIPPED
		...
		struct rte_mbuf **rx_pkts)
{
	...
	/* shift left by NGBE_PACKET_TYPE_SHIFT, and apply ptype mask */
	ptype0 = _mm_and_si128(_mm_srli_epi32(ptype0, NGBE_RXD_PTID_SHIFT),
			       ptype_mask);

	pkt_info = _mm_extract_epi32(ptype0, 0);
	rx_pkts[0]->packet_type =
		ngbe_rxd_pkt_info_to_pkt_type(pkt_info, pkt_type_mask);
 ...
 * - nb_pkts > RTE_NGBE_MAX_RX_BURST, only scan RTE_NGBE_MAX_RX_BURST
 *   numbers of DD bit
		...
		uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	/* nb_pkts shall be less equal than RTE_NGBE_MAX_RX_BURST */
	nb_pkts = RTE_MIN(nb_pkts, RTE_NGBE_MAX_RX_BURST);
			   ...
			   uint16_t nb_pkts)
{
	...
	uint64_t rs = NGBE_TXD_FLAGS; //NGBE_TXD_RS | NGBE_TXD_FLAGS;
	int i;
