...
struct ngbe_eth_l2_tunnel_conf_ele {
	...
	struct ngbe_l2_tunnel_conf filter_info;
};
...
cons_parse_l2_tn_filter(struct rte_eth_dev *dev,
			...
			const struct rte_flow_item pattern[],
			const struct rte_flow_action actions[],
			struct ngbe_l2_tunnel_conf *filter,
			struct rte_flow_error *error)
{
	const struct rte_flow_item *item;
	...
	if (item->type != RTE_FLOW_ITEM_TYPE_E_TAG) {
		memset(filter, 0, sizeof(struct ngbe_l2_tunnel_conf));
		rte_flow_error_set(error, EINVAL,
			RTE_FLOW_ERROR_TYPE_ITEM,
...
ngbe_parse_l2_tn_filter(struct rte_eth_dev *dev,
			...
			const struct rte_flow_item pattern[],
			const struct rte_flow_action actions[],
			struct ngbe_l2_tunnel_conf *l2_tn_filter,
			struct rte_flow_error *error)
{
	int ret = 0;
	struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
		  ...
		  struct rte_flow_error *error)
{
	...
	memset(&l2_tn_filter, 0, sizeof(struct ngbe_l2_tunnel_conf));
	...
	if (!ret) {
		...
		if (!ret) {
			...
			rte_memcpy(&l2_tn_filter_ptr->filter_info,
				&l2_tn_filter,
				sizeof(struct ngbe_l2_tunnel_conf));
