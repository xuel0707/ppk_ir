 ...
 * Register itself as the [Poll Mode] Driver of PCI NGBE devices.
 */
static int
rte_ngbe_pmd_init(const char *name __rte_unused, const char *params __rte_unused)
{
	PMD_INIT_FUNC_TRACE();

	rte_eth_driver_register(&rte_ngbe_pmd);
	return 0;
}
...
static struct rte_driver rte_ngbe_driver = {
	.type = PMD_PDEV,
	.init = rte_ngbe_pmd_init,
};

PMD_REGISTER_DRIVER(rte_ngbe_driver, ngbe);
DRIVER_REGISTER_PCI_TABLE(ngbe, pci_id_ngbe_map);