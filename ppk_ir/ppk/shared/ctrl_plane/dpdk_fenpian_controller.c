...
void fill_section_table_table(uint8_t isvalid)
{
    ...
    strcpy(te->table_name, "IngressImpl.section_table_0");
    ...
    strcpy(a->description.name, "IngressImpl.readSection");
...
void fill_count_table_table(uint8_t isvalid[5], uint8_t isvalid5)
{
    ...
    h = create_p4_header(buffer, 0, 2048);
    te = create_p4_add_table_entry(buffer, 0, 2048);
    strcpy(te->table_name, "IngressImpl.count_table_0");
    ...
    a = add_p4_action(h, 2048);
    strcpy(a->description.name, "IngressImpl.storeNumOfComs");
    ...
    send_p4_msg(c, buffer, 2048);
    fp6 = fopen("/opt/Workspace/P4/fenpian.txt","a");
                if(fp6){
                    fprintf(fp6,"2222222:%d ,%d ,%d ,%d ,%d,%d",isvalid[0],isvalid[1],isvalid[2],isvalid[3],isvalid[4],isvalid5);
                    fprintf(fp6,"\n");
                    fclose(fp6);
                }
    usleep(1200);
...
void fill_fib_table_table(uint8_t isvalid, uint16_t isvalid1[5], uint16_t mask[5], uint8_t isvalid2)
{
    char buffer[4096];
    struct p4_header *h;
    struct p4_add_table_entry *te;
    ...
    struct p4_action_parameter *ap;
    struct p4_field_match_exact *exact;
    struct p4_field_match_ternary *ternary, *ternary1, *ternary2, *ternary3, *ternary4;

    // fp6 = fopen("/home/zhaoxin/log/fenpian.txt","a");
    //             if(fp6){
    //                 fprintf(fp6,"2222222:%x %x %x %x",isvalid1[2],isvalid1[4],mask[0],mask[4]);
    //                 fprintf(fp6,"\n");
    //                 fclose(fp6);
    //             }
    ...
    strcpy(te->table_name, "IngressImpl.fib_table_0");

    exact = add_p4_field_match_exact(te, 4096);
    strcpy(exact->header.name, "meta.components");
    memcpy(exact->bitmap, &isvalid, 1);
    exact->length = 8 * 1 + 0;

    ternary = add_p4_field_match_ternary(te, 4096);
    strcpy(ternary->header.name, "meta.c1");
    memcpy(ternary->bitmap, &isvalid1[0], 2);
    memcpy(ternary->mask, &mask[0], 2);

    ternary1 = add_p4_field_match_ternary(te, 4096);
    strcpy(ternary1->header.name, "meta.c2");
    memcpy(ternary1->bitmap, &isvalid1[1], 2);
    memcpy(ternary1->mask, &mask[1], 2);

    ternary2 = add_p4_field_match_ternary(te, 4096);
    strcpy(ternary2->header.name, "meta.c3");
    memcpy(ternary2->bitmap, &isvalid1[2], 2);
    memcpy(ternary2->mask, &mask[2], 2);

    ternary3 = add_p4_field_match_ternary(te, 4096);
    strcpy(ternary3->header.name, "meta.c4");
    memcpy(ternary3->bitmap, &isvalid1[3], 2);
    memcpy(ternary3->mask, &mask[3], 2);

    ternary4 = add_p4_field_match_ternary(te, 4096);
    strcpy(ternary4->header.name, "meta.name_hash");
    memcpy(ternary4->bitmap, &isvalid1[4], 2);
    memcpy(ternary4->mask, &mask[4], 2);

    a = add_p4_action(h, 4096);
    strcpy(a->description.name, "IngressImpl.set_egr");
...
void fill_pit_table_table(char isvalid, uint8_t is_valid)
{
    ...
    h = create_p4_header(buffer, 0, 2048);
    te = create_p4_add_table_entry(buffer, 0, 2048);
    strcpy(te->table_name, "IngressImpl.pit_table_0");
    ...
    {
        a = add_p4_action(h, 2048);
        strcpy(a->description.name, "IngressImpl.readPitEntry");
        netconv_p4_header(h);
        netconv_p4_add_table_entry(te);
        netconv_p4_field_match_exact(exact);
        netconv_p4_action(a);
    ...
    else if (is_valid == 1)
    {
        ...
        strcpy(a1->description.name, "IngressImpl.cleanPitEntry");
        netconv_p4_header(h);
        netconv_p4_add_table_entry(te);
        netconv_p4_field_match_exact(exact);
...
void fill_updatecs_table_table(uint8_t isvalid)
{
    ...
    h = create_p4_header(buffer, 0, 2048);
    te = create_p4_add_table_entry(buffer, 0, 2048);
    strcpy(te->table_name, "IngressImpl.updatecs_table_0");
    ...
    {
        a1 = add_p4_action(h, 2048);
        strcpy(a1->description.name, "IngressImpl.updateCsEntry");
        netconv_p4_header(h);
        netconv_p4_add_table_entry(te);
        netconv_p4_field_match_exact(exact);
        netconv_p4_action(a1);
    ...
    {
        a = add_p4_action(h, 2048);
        strcpy(a->description.name, "IngressImpl.addCstoMutist");
        netconv_p4_header(h);
        netconv_p4_add_table_entry(te);
        netconv_p4_field_match_exact(exact);
        netconv_p4_action(a);
...
void fill_updatePit_table_table(uint8_t isvalid)
{
    ...
    h = create_p4_header(buffer, 0, 2048);
    te = create_p4_add_table_entry(buffer, 0, 2048);
    strcpy(te->table_name, "IngressImpl.updatePit_table_0");
    ...
    a = add_p4_action(h, 2048);
    strcpy(a->description.name, "IngressImpl.updatePit_entry");
...
void set_default_action_section_table()
{
    ...
    strcpy(sda->table_name, "IngressImpl.section_table_0");

    a = &(sda->action);
    strcpy(a->description.name, "IngressImpl._drop");
...
void set_default_action_fib_table()
{
    ...
    sda = create_p4_set_default_action(buffer, 0, sizeof(buffer));
    strcpy(sda->table_name, "IngressImpl.fib_table_0");
...
void set_default_action_updatePit_table()
{
    ...
    sda = create_p4_set_default_action(buffer, 0, sizeof(buffer));
    strcpy(sda->table_name, "IngressImpl.updatePit_table_0");
...
int read_config_from_file(char *filename)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        else if (line[0] == 'C')
        {
            ...
            //          fclose(fp6);
            //      }
            if (13 == sscanf(line, "%c %hhd %x %x %x %x %x %x %x %x %x %x %hhd", &dummy, &isvalid, &isvalid1[0], &isvalid1[1], &isvalid1[2], &isvalid1[3], &isvalid1[4], &mask[0], &mask[1], &mask[2], &mask[3], &mask[4], &isvalid2))
            {
                // fp6 = fopen("/home/zhaoxin/log/fenpian.txt","a");
                // if(fp6){
                //     fprintf(fp6,"bbb:%x %x %x %x",isvalid1[2],isvalid1[4],mask[0],mask[4]);
                //     fprintf(fp6,"\n");
                //     fclose(fp6);
                // }
                fill_fib_table_table(isvalid, isvalid1, mask, isvalid2);
            }
            else
            {
                fclose(f);
