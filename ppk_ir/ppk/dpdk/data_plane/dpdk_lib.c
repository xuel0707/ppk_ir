...
{
    // debug("dpdk_lib.c: Where get_socketid() begins, and the value of numa_on is %d \n",numa_on);
    int socketid = numa_on ? rte_lcore_to_socket_id(lcore_id) : 0; //获得指定lcore的物理socket_id
...
packet* copy_packet(packet* pd, struct rte_mempool* mempool, uint32_t offset, uint32_t length) {
    return rte_pktmbuf_copy(pd, mempool, 0, UINT32_MAX);
}