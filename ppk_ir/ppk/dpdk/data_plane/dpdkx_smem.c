...
void set_direct_counter(direct_counter_t* smem, uint32_t value, char* table_name, char* smem_type_name, char* smem_name) {
    rte_atomic32_set(&(smem->value), value);
    debug("    : Set " T4LIT(%s) " " T4LIT(%s,smem) "(" T4LIT(%d) ") on table " T4LIT(%s,table) ": new value is " T4LIT(%d) "\n",
          smem_type_name, smem_name, value, table_name, rte_atomic32_read(&(smem->value)));
}

void read_direct_counter(direct_counter_t* smem, char* table_name, char* smem_type_name, char* smem_name, uint32_t *value) {
    *value = rte_atomic32_read(&(smem->value));
    debug("    : Read " T4LIT(%s) " " T4LIT(%s,smem) "(" T4LIT(%d) ") on table " T4LIT(%s,table) ": the value is " T4LIT(%d) "\n",
          smem_type_name, smem_name, value, table_name, rte_atomic32_read(&(smem->value)));
}
...
void extern_counter_set(counter_t *counter, int index, uint32_t value) {
    rte_atomic32_set(&(counter->value), value);
    #ifdef PPK_DEBUG
        debug("    : Set counter to " T4LIT(%3d) ": " T4LIT(%s[%d],smem) " = " T4LIT(%d,bytes) "\n", value, counter->name, index, rte_atomic32_read(&(counter->value)));
    #endif
}

void extern_counter_read(counter_t* counter, int index, uint32_t *value) {
    *value = rte_atomic32_read(&(counter->value));
    #ifdef PPK_DEBUG
        // printf("    : Read counter from " T4LIT(%s[%d],smem) " = " T4LIT(%d,bytes) "\n", counter->name, index, *value);
    #endif
}
