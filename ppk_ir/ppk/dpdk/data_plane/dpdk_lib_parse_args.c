...
/*****************************************************
 ** mbufs :从内核接收的数据包数据
 kni_port_id:kni网卡端口号
max_rx_kni_pkt:当前可接收最大数据包个数,不可以超过32
 返回值：从内核接收到的数据包个数，一次最多32个
 ***********************************************************/
unsigned my_kni_egress(struct rte_mbuf **mbufs, uint16_t kni_port_id, uint16_t max_rx_kni_pkt)
{
	uint8_t i;
	uint16_t port_id;
	unsigned num;
	...
	{
		/* Burst rx from kni */
		num = rte_kni_rx_burst(p->kni[i], (void **)mbufs, max_rx_kni_pkt);
		if (unlikely(num > max_rx_kni_pkt))
		{
