...
uint16_t ppk_nb_rxd = RTE_TEST_RX_DESC_DEFAULT;
uint16_t ppk_nb_txd = RTE_TEST_TX_DESC_DEFAULT;
...
bool check_port_config(unsigned nb_ports)
{
    ...
    for (uint16_t i = 0; i < nb_lcore_params; ++i) {
        ...
        debug (" dpdk_lib_init_hw.c: The value of nb_ports is %u \n ",nb_ports);
        if (portid >= nb_ports) {
...
void init_mbuf_pool(int socketid)
{
    ...
    if (pktmbuf_pool[socketid] == NULL)
        rte_exit(EXIT_FAILURE, "Cannot init mbuf pool[%s] on socket %d\n",s, socketid);
}
...
bool init_tx_on_lcore(unsigned lcore_id, uint8_t portid, uint16_t queueid)
{
    ...
    int ret = rte_eth_tx_queue_setup(portid, queueid, ppk_nb_txd, socketid, txconf);
    if (ret < 0)
...
void dpdk_init_rx_queue(uint8_t queue, unsigned lcore_id, struct lcore_conf* qconf) {
    ...
    int ret = rte_eth_rx_queue_setup(portid, queueid, ppk_nb_rxd,
...
void dpdk_init_nic()
{
    debug("dpdk-lib-init-hw.c: Begin to dpdk_init_nic()\n");
    ...
    #if KNI_STORAGE
        uint32_t nb_lcores = min(rte_lcore_count(),2);
    #else
    ...
    reset_mbuf_pools();
        ...
        for (unsigned lcore_id = 0; lcore_id < nb_lcores; lcore_id++) {
            ...
            int socketid = get_socketid(lcore_id);
            //printf("ssz debug : socketid:%d    lcore_id:%d\n",socketid, lcore_id);
