...
void ternary_add(lookup_table_t* t, uint8_t* key, uint8_t* mask,uint16_t priority,uint8_t* value)
{
    if (t->entry.key_size == 0) return; // don't add lines to keyless tables

    uint8_t* entry = make_table_entry_on_socket(t, value);
    naive_ternary_add(t->table, key, mask,priority,entry);
}
...
uint8_t* ternary_lookup(lookup_table_t* t, uint8_t* key)
{
    ...
    /*fp = fopen("/home/it-34/log/log1.txt","a");
    fprintf(fp,"11111");
    fprintf(fp,"\n++++++++++++");
    fclose(fp);*/
...
void ternary_delete(lookup_table_t* t, uint8_t* key)
{
    if (t->entry.key_size == 0) return; 
    uint8_t* ret = naive_ternary_lookup(t->table, key);
    if(ret==NULL)  return ;      //语法错误
    else 
        rte_free(ret);
}
//////////////////////Xiajiqiang 2020/09/22//////////////////
