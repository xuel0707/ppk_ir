...
void transfer_to_egress(STDPARAMS)
{
    ...
    (void)res32;
    #if PPK_VLAN
    MODIFY_INT32_INT32_BITS_PACKET(pd, header_instance_ethernet, field_ethernet_t_ether_type, 0x0081);
    debug("       v1model :value of ethernet-type field  " T4LIT(% 08x) "\n", GET_INT32_AUTO_PACKET(pd, header_instance_ethernet, field_ethernet_t_ether_type));
    /*TODO repair reorder header bool value*/
