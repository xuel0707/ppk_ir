...
#define FORALL_PRINTOUT(txt1, txt2, b, should_print) \
    ...
    if (should_print) { \
        dbg_bytes(key, state[socketid].tables[tableid][0]->entry.key_size, " " T4LIT(ctl>,incoming) " #" T4LIT(txt1,action) " " T4LIT(%s,table) txt2 ": <- ", table_config[tableid].name); \
    }
#else
#define FORALL_PRINTOUT(txt1, txt2, b, should_print)
#endif
...
void lpm_add_promote(int tableid, uint8_t* key, uint8_t depth, uint8_t* value, bool should_print) {
    // static int be_called = 0;
    // ++be_called;
    // printf("流表添加成功%d次\n",be_called);
    // printf("key_:{%d}{%d}{%d}{%d}{%d}{%d}{%d}{%d}{%d}{%d}",key[0],key[1],key[2],key[3],key[4],key[5],key[6],key[7],key[8],key[9]);
    FORALLNUMANODES(Add, "/" T4LIT(LPM), CHANGE_TABLE(lpm_add, key, depth, value), should_print)
}
void ternary_add_promote(int tableid, uint8_t* key, uint8_t* mask , uint16_t priority, uint8_t* value, bool should_print) {
    FORALLNUMANODES(Add, "/" T4LIT(ternary), CHANGE_TABLE(ternary_add, key, mask, priority, value), should_print)
}
...
void exact_lookup_promote(int tableid, uint8_t* key, bool should_print) {
    FORALLNUMANODES(Lookup, "/" T4LIT(exact), CHANGE_TABLE(exact_lookup, key), should_print)
}
void lpm_lookup_promote(int tableid, uint8_t* key, bool should_print) {
    FORALLNUMANODES(Lookup, "/" T4LIT(LPM), CHANGE_TABLE(lpm_lookup, key), should_print)
}
void ternary_lookup_promote(int tableid, uint8_t* key, bool should_print) {
    FORALLNUMANODES(Lookup, "/" T4LIT(ternary), CHANGE_TABLE(ternary_lookup, key), should_print)
}

void exact_delete_promote(int tableid, uint8_t* key, bool should_print) {
    FORALLNUMANODES(Delete, "/" T4LIT(exact), CHANGE_TABLE(exact_delete, key), should_print)
}
void lpm_delete_promote(int tableid, uint8_t* key, uint8_t depth, bool should_print) {
    //printf("lpm_delete_promote里面+++++++++++++++++++++++++++\n");
    FORALLNUMANODES(Delete, "/" T4LIT(LPM), CHANGE_TABLE(lpm_delete, key), should_print)
}
void ternary_delete_promote(int tableid, uint8_t* key, bool should_print) {
    FORALLNUMANODES(Delete, "/" T4LIT(ternary), CHANGE_TABLE(ternary_delete, key), should_print)
}
