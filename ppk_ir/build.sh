...
candidate_count() {
    simple_count=$(find "$IR_SRC_DIR" -type f -name "$1.json*" | wc -l)
    if [ $simple_count -eq 1 ]; then
        echo 1
    else
        echo $(find "$IR_SRC_DIR" -type f -name "*$1*.json*" | wc -l)
    fi
}
...
candidates() {
    if [ $(candidate_count $1) -gt 0 ]; then
        echo
        find "$IR_SRC_DIR" -type f -name "*$1*.json*" | sed 's#^.*/\([^\.]*\).*$#    \1#g'
    else
...
#            echo "NEWOPTS: ${NEWOPTS[@]}"
        ...
        for opt in ${NEWOPTS[@]}; do
            ...
            if [ "$(array_contains "${groups[prefix]}" ":" "::" "%" "%%")" == y ]; then
                ...
                setopt example "$var"
                setopt source "`find "$IR_SRC_DIR" -type f -name "${var}.json*"`"
            fi
...
if [ "$(optvalue p4)" != off ]; then
    ...
    addopt ir_opts "${OPTS[source]}" " "
    addopt ir_opts "-g ${PPK_SRCGEN_DIR}" " "
    # addopt ir_opts "-desugar_info none" " "
    [ "$(optvalue verbose)" != off ] && addopt ir_opts "-verbose" " "

    verbosemsg "IR compiler options: $(print_cmd_opts "${OPTS[ir_opts]}")"

    IFS=" "
    echo ${OPTS[ir_opts]}
    $PYTHON -B compiler/compiler.py ${OPTS[ir_opts]}
    exit_on_error "IR to C compilation failed"
fi
