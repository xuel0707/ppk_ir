		     ...
		     uint32_t *fdirctrl, uint32_t *flex)
{
	...
	*flex |= TXGBE_FDIRFLEXCFG_OFST(TXGBE_DEFAULT_FLEXBYTES_OFFSET / 2);
	...
	case RTE_FDIR_MODE_PERFECT_MAC_VLAN:
		//*fdirctrl |= (TXGBE_FDIRCTL_FILTERMODE_MACVLAN);
		//*fdirctrl |= TXGBE_FDIRCTL_DROPQP(conf->drop_queue);
		//break;
	case RTE_FDIR_MODE_PERFECT_TUNNEL:
		//*fdirctrl |= (TXGBE_FDIRCTL_FILTERMODE_CLOUD);
		//*fdirctrl |= TXGBE_FDIRCTL_DROPQP(conf->drop_queue);
		//break;
	default:
		/* bad value */
...
txgbe_fdir_store_input_mask(struct rte_eth_dev *dev)
{
	...
	if (info->mask.src_port_mask & info->mask.dst_port_mask &
		info->mask.src_ipv4_mask & info->mask.dst_ipv4_mask)
		info->mask_added = FALSE;
	else
		info->mask_added = TRUE;
...
txgbe_set_fdir_flex_conf(struct rte_eth_dev *dev, uint32_t flex)
{
	...
	for (i = 0; i < conf->nb_flexmasks; i++) {
		...
		if (flexbytes == UINT16_MAX) {
			...
			/* TXGBE_FDIRM_FLEX is set by default when set mask */
		...
		struct txgbe_atr_input *input, enum rte_fdir_mode mode)
{
	...
	case RTE_ETH_FLOW_NONFRAG_IPV6_OTHER:
		...
		break;
	default:
		ptype = 0; //txgbe_ptype_table[TXGBE_PT_ETHER];
		break;
	}
...
/*
 * The below function is taken from the FreeBSD TXGBE drivers release
 * 2.3.8. The only change is not to mask hash_result with TXGBE_ATR_HASH_MASK
 ...
 * base/txgbe_hw.c, with the ability to set extra flags in FDIRCMD register
...
/**
 * This function is based on txgbe_atr_add_signature_filter_raptor() in
