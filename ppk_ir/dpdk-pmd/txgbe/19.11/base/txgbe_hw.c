...
bool txgbe_device_supports_autoneg_fc(struct txgbe_hw *hw)
{
	...
	case txgbe_media_type_fiber:
		...
		switch (hw->device_id) {
//x550: 	case TXGBE_DEV_ID_X550EM_A_SFP:
//			supported = false;
//			break;
		default:
...
//	ctrl_ext = rd32(hw, TXGBE_CTRL_EXT);
//	ctrl_ext |= TXGBE_CTRL_EXT_NS_DIS;
//	wr32(hw, TXGBE_CTRL_EXT, ctrl_ext);
//	txgbe_flush(hw);
...
//fixme:/* Disable relaxed ordering */
//	for (i = 0; i < hw->mac.max_tx_queues; i++) {
//		regval = rd32(hw, TXGBE_DCA_TXCTRL_82599(i));
//		regval &= ~TXGBE_DCA_TXCTRL_DESC_WRO_EN;
//		wr32(hw, TXGBE_DCA_TXCTRL_82599(i), regval);
//	}
//
//	for (i = 0; i < hw->mac.max_rx_queues; i++) {
//		regval = rd32(hw, TXGBE_DCA_RXCTRL(i));
//		regval &= ~(TXGBE_DCA_RXCTRL_DATA_WRO_EN |
//			    TXGBE_DCA_RXCTRL_HEAD_WRO_EN);
//		wr32(hw, TXGBE_DCA_RXCTRL(i), regval);
//	}
...
s32 txgbe_init_hw(struct txgbe_hw *hw)
{
	...
	hw->phy.get_firmware_version(hw, &hw->fw_version);
...
s32 txgbe_clear_hw_cntrs(struct txgbe_hw *hw)
{
	...
	/* MNG Stats */
	//rd32(hw, TXGBE_MNGPRC);
	//rd32(hw, TXGBE_MNGPDC);
	//rd32(hw, TXGBE_MNGPTC);
...
//x550: if (hw->phy.id == 0)
//		txgbe_identify_phy(hw);
//	hw->phy.read_reg(hw, TXGBE_PCRC8ECL,
//			     TXGBE_MD_DEV_PCS, &i);
//	hw->phy.read_reg(hw, TXGBE_PCRC8ECH,
//			     TXGBE_MD_DEV_PCS, &i);
//	hw->phy.read_reg(hw, TXGBE_LDPCECL,
//			     TXGBE_MD_DEV_PCS, &i);
//	hw->phy.read_reg(hw, TXGBE_LDPCECH,
//			     TXGBE_MD_DEV_PCS, &i);
...
s32 txgbe_led_on(struct txgbe_hw *hw, u32 index)
{
	...
	led_reg |= index << TXGBE_LEDCTL_OD_SHIFT;
	led_reg |= index;
...
STATIC s32 txgbe_fc_autoneg_fiber(struct txgbe_hw *hw)
{
	 ...
	 */
#if 0
	linkstat = rd32(hw, TXGBE_PCS1GLSTA);
	if ((!!(linkstat & TXGBE_PCS1GLSTA_AN_COMPLETE) == 0) ||
	    (!!(linkstat & TXGBE_PCS1GLSTA_AN_TIMED_OUT) == 1)) {
...
STATIC s32 txgbe_fc_autoneg_backplane(struct txgbe_hw *hw)
{
	...
	links = rd32_epcs(hw, VR_AN_INTR_MSK);
	if ((links & VR_AN_INTR_CMPLT_IE) == 0) {
		DEBUGOUT("Auto-Negotiation did not complete\n");
		return err;
	}

	if (hw->mac.type == txgbe_mac_raptor) {
		links2 = rd32(hw, TXGBE_LINKS2);
		if ((links2 & TXGBE_LINKS2_AN_SUPPORTED) == 0) {
...
s32 txgbe_disable_pcie_master(struct txgbe_hw *hw)
{
	...
	DEBUGFUNC("txgbe_disable_pcie_master");
#if 0 /* fixme: firmware interface */
	/* Always set this bit to ensure any future transactions are blocked */
	wr32(hw, TXGBE_CTRL, TXGBE_CTRL_GIO_DIS);

	/* Exit if master requests are blocked */
	if (!(rd32(hw, TXGBE_STATUS) & TXGBE_STATUS_GIO) ||
...
#else
	return status;
#endif /* fixme: firmware interface */
	/*
	...
	//x550: if (hw->mac.type >= txgbe_mac_X550)
	//	goto out;
	...
	for (i = 0; i < poll; i++) {
		usec_delay(100);
		//value = TXGBE_READ_PCIE_WORD(hw, TXGBE_PCI_DEVICE_STATUS);
		//if (TXGBE_REMOVED(hw->hw_addr))
		//	goto out;
		//if (!(value & TXGBE_PCI_DEVICE_STATUS_TRANSACTION_PENDING))
		//	goto out;
	}
...
s32 txgbe_acquire_swfw_sync(struct txgbe_hw *hw, u32 mask)
{
	...
	for (i = 0; i < timeout; i++) {
		...
		gssr = rd32(hw, TXGBE_MNGSEM);
		if (!(gssr & (fwmask | swmask))) {
			gssr |= swmask;
			wr32(hw, TXGBE_MNGSEM, gssr);
...
/**
 *  txgbe_enable_relaxed_ordering_gen2 - Enable relaxed ordering
 *  @hw: pointer to hardware structure
 *
 **/
...
//	u32 i;
	...
	DEBUGFUNC("txgbe_enable_relaxed_ordering_gen2");
...
void txgbe_clear_tx_pending(struct txgbe_hw *hw)
{
	...
	//fixme:gcr_ext = rd32(hw, TXGBE_GCR_EXT);
	//wr32(hw, TXGBE_GCR_EXT,
	//		gcr_ext | TXGBE_GCR_EXT_BUFFERS_CLEAR);
...
s32 txgbe_setup_sfp_modules(struct txgbe_hw *hw)
{
	...
	//err = txgbe_get_sfp_init_sequence_offsets(hw, &list_offset,
	//					      &data_offset);
	//if (err != 0)
	//	return err;
	...
	//	hw->mac.orig_autoc | TXGBE_AUTOC_LMS_10Gs,
	//	false);
...
s32 txgbe_init_ops_pf(struct txgbe_hw *hw)
{
	...
	mac->enable_relaxed_ordering = txgbe_enable_relaxed_ordering_gen2;
	...
	//delete me: txgbe_init_mac_link_ops(hw);
	mac->rar_highwater = 1;
...
/**
 *  txgbe_stop_mac_link_on_d3_raptor - Disables link on D3
 *  @hw: pointer to hardware structure
 *
 *  Disables link during D3 power down sequence.
 *
 **/
void txgbe_stop_mac_link_on_d3_raptor(struct txgbe_hw *hw)
{
#if 0
	u32 autoc2_reg;
	u16 ee_ctrl_2 = 0;

	DEBUGFUNC("txgbe_stop_mac_link_on_d3_raptor");
	hw->rom.read16(hw, TXGBE_EEPROM_CTRL_2, &ee_ctrl_2);

	if (!txgbe_mng_present(hw) && !hw->wol_enabled &&
	    ee_ctrl_2 & TXGBE_EEPROM_CCD_BIT) {
		autoc2_reg = rd32(hw, TXGBE_AUTOC2);
		autoc2_reg |= TXGBE_AUTOC_LINK_DSA_D3_MASK;
		wr32(hw, TXGBE_AUTOC2, autoc2_reg);
	}
#else
	UNREFERENCED_PARAMETER(hw);
	//fixme
...
//		autoc_reg = rd32_epcs(hw, SR_AN_MMD_ADV_REG1);
//		if ((autoc_reg & TXGBE_AUTOC_LMS_MASK) ==
//		     TXGBE_AUTOC_LMS_KX4_KX_KR ||
//		    (autoc_reg & TXGBE_AUTOC_LMS_MASK) ==
//		     TXGBE_AUTOC_LMS_KX4_KX_KR_1G_AN ||
//		    (autoc_reg & TXGBE_AUTOC_LMS_MASK) ==
//		     TXGBE_AUTOC_LMS_KX4_KX_KR_SGMII) {
//			links_reg = 0; /* Just in case Autoneg time = 0 */
//			for (i = 0; i < TXGBE_AUTO_NEG_TIME; i++) {
//				links_reg = rd32(hw, TXGBE_PORTSTAT);
//				if (links_reg & TXGBE_LINKS_KX_AN_COMP)
//					break;
//				msec_delay(100);
//			}
//			if (!(links_reg & TXGBE_LINKS_KX_AN_COMP)) {
...
void txgbe_set_fdir_drop_queue_raptor(struct txgbe_hw *hw, u8 dropqueue)
{
	...
	//x550: fdirctrl |= TXGBE_FDIRCTRL_DROP_NO_MATCH;
...
//	u32 fdirip6m;
	...
	switch (input_mask->flex_bytes & 0xFFFF) {
//	case 0x0000:
//		/* Mask Flex Bytes */
//		fdirm |= TXGBE_FDIRM_FLEX;
//		/* fall through */
//	case 0xFFFF:
//		break;
	default:
...
//	if (cloud_mode) {
//		fdirm |= TXGBE_FDIRM_L3P;
//		fdirip6m = ((u32) 0xFFFFU << TXGBE_FDIRIP6M_DIPM_SHIFT);
//		fdirip6m |= TXGBE_FDIRIP6M_ALWAYS_MASK;
//
//		switch (input_mask->inner_mac[0] & 0xFF) {
//		case 0x00:
//			/* Mask inner MAC, fall through */
//			fdirip6m |= TXGBE_FDIRIP6M_INNER_MAC;
//		case 0xFF:
//			break;
//		default:
...
//		}
//
//		switch (input_mask->tni_vni & BIT_MASK32) {
//		case 0x0:
//			/* Mask vxlan id */
//			fdirip6m |= TXGBE_FDIRIP6M_TNI_VNI;
//			break;
//		case 0x00FFFFFF:
//			fdirip6m |= TXGBE_FDIRIP6M_TNI_VNI_24;
//			break;
//		case 0xFFFFFFFF:
//			break;
//		default:
...
//		}
//
//		switch (input_mask->tunnel_type & 0xFFFF) {
//		case 0x0:
//			/* Mask turnnel type, fall through */
//			fdirip6m |= TXGBE_FDIRIP6M_TUNNEL_TYPE;
//		case 0xFFFF:
//			break;
//		default:
...
//			return TXGBE_ERR_CONFIG;
//		}
//		wr32(hw, TXGBE_FDIRIP6MSK, npu_to_be32(fdirip6m));
//
//		/* Set all bits in FDIRTCPM, FDIRUDPM, FDIRSCTPM,
//		 * FDIRSIP4M and FDIRDIP4M in cloud mode to allow
//		 * L3/L3 packets to tunnel.
//		 */
//		wr32(hw, TXGBE_FDIRTCPMSK, 0xFFFFFFFF);
//		wr32(hw, TXGBE_FDIRUDPMSK, 0xFFFFFFFF);
//		wr32(hw, TXGBE_FDIRDIP4MSK, 0xFFFFFFFF);
//		wr32(hw, TXGBE_FDIRSIP4MSK, 0xFFFFFFFF);
//		wr32(hw, TXGBE_FDIRSCTPMSK, 0xFFFFFFFF);
//	}
...
//	if (cloud_mode) {
//		if (input->tunnel_type != 0)
//			cloud_type = 0x80000000;
//
//		addr_low = ((u32)input->inner_mac[0] |
//				((u32)input->inner_mac[1] << 8) |
//				((u32)input->inner_mac[2] << 16) |
//				((u32)input->inner_mac[3] << 24));
//		addr_high = ((u32)input->inner_mac[4] |
//				((u32)input->inner_mac[5] << 8));
//		cloud_type |= addr_high;
//		TXGBE_WRITE_REG_BE32(hw, TXGBE_FDIRPISIP6(0), addr_low);
//		TXGBE_WRITE_REG_BE32(hw, TXGBE_FDIRPISIP6(1), cloud_type);
//		TXGBE_WRITE_REG_BE32(hw, TXGBE_FDIRPISIP6(2), input->tni_vni);
//	}
...
/**
 * txgbe_fw_recovery_mode - Check FW NVM recovery mode
 * @hw: pointer t hardware structure
 *
 * Returns true if in FW NVM recovery mode.
 **/
//bool txgbe_fw_recovery_mode(struct txgbe_hw *hw)
//{
//	u32 fwsm;
//
//	fwsm = rd32(hw, TXGBE_FWSM_BY_MAC(hw));
//
//	return !!(fwsm & TXGBE_FWSM_FW_NVM_RECOVERY_MODE);
//}
