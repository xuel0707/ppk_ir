	...
	struct rte_mbuf **rx_pkts)
{
	...
	const __m128i vlan_csum_msk = _mm_set_epi16(
		...
		TXGBE_RXD_STAT_VLAN, TXGBE_RXD_STAT_VLAN);
	//fixme: need to check both TXGBE_RXD_STAT_VLAN and PKT_RX_VLAN_STRIPPED
		...
		struct rte_mbuf **rx_pkts)
{
	...
	/* shift left by TXGBE_PACKET_TYPE_SHIFT, and apply ptype mask */
	ptype0 = _mm_and_si128(_mm_srli_epi32(ptype0, TXGBE_RXD_PTID_SHIFT),
			       ptype_mask);

	pkt_info = _mm_extract_epi32(ptype0, 0);
	rx_pkts[0]->packet_type =
		txgbe_rxd_pkt_info_to_pkt_type(pkt_info, pkt_type_mask);
