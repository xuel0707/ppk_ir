 ...
 * Typical work flow:
 *
 * IF NOT ngbe_ptype_lookup[ptid].known
 * THEN
 *      Packet is unknown
 * ELSE IF ngbe_ptype_lookup[ptid].mac == NGBE_DEC_PTYPE_MAC_IP
...
static inline u8
ngbe_encode_ptype_fcoe(u32 ptype)
{
	u8 ptid;

	UNREFERENCED_PARAMETER(ptype);
	ptid = NGBE_PTID_PKT_FCOE;
...
ngbe_encode_ptype_mac(u32 ptype)
{
	...
	case RTE_PTYPE_L2_ETHER_FCOE:
		ptid = ngbe_encode_ptype_fcoe(ptype);
		break;
