 ...
 */
static int
rte_ngbevf_pmd_init(const char *name __rte_unused, const char *param __rte_unused)
{
	PMD_INIT_FUNC_TRACE();

	rte_eth_driver_register(&rte_ngbevf_pmd);
	return 0;
}
...
static struct rte_driver rte_ngbevf_driver = {
	.type = PMD_PDEV,
	.init = rte_ngbevf_pmd_init,
};

PMD_REGISTER_DRIVER(rte_ngbevf_driver, ngbevf);
DRIVER_REGISTER_PCI_TABLE(ngbevf, pci_id_ngbevf_map);
