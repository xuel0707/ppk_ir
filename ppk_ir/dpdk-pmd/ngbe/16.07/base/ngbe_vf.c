...
static void ngbe_virt_clr_reg(struct ngbe_hw *hw)
{
	...
	//fixme: clear NGBE_VFRSSKEY(i)
	//fixme: clear NGBE_VFRSSTBL(i)
				 ...
				 bool clear)
{
	...
	DEBUGFUNC("ngbe_update_mc_addr_list_vf");
 ...
 **/
u32 ngbe_get_num_of_tx_queues_vf(struct ngbe_hw *hw)
{
 ...
 **/
u32 ngbe_get_num_of_rx_queues_vf(struct ngbe_hw *hw)
{
	UNREFERENCED_PARAMETER(hw);
 ...
 **/
s32 ngbe_setup_mac_link_vf(struct ngbe_hw *hw, u32 speed,
...
s32 ngbe_init_ops_vf(struct ngbe_hw *hw)
{
	...
	/* Link */
	mac->setup_link = ngbe_setup_mac_link_vf;
