...
eth_ngbevf_dev_init(struct rte_eth_dev *eth_dev)
{
	int err;
	uint32_t tc, tcs;
	struct ngbe_adapter *ad = eth_dev->data->dev_private;
...
ngbevf_dev_promiscuous_enable(struct rte_eth_dev *dev)
{
	...
	err = hw->mac.update_xcast_mode(hw, NGBEVF_XCAST_MODE_PROMISC);
	switch (err) {
	case 0:
		err = 0;
		break;
...
ngbevf_dev_promiscuous_disable(struct rte_eth_dev *dev)
{
	...
	err = hw->mac.update_xcast_mode(hw, NGBEVF_XCAST_MODE_NONE);
	switch (err) {
	case 0:
		err = 0;
		break;
...
{
	struct ngbe_hw *hw = NGBE_DEV_HW(dev);
	int mode = NGBEVF_XCAST_MODE_ALLMULTI;
	int err;

	err = hw->mac.update_xcast_mode(hw, mode);
	switch (err) {
	case 0:
		err = 0;
		break;
...
ngbevf_dev_allmulticast_disable(struct rte_eth_dev *dev)
{
	...
	err = hw->mac.update_xcast_mode(hw, NGBEVF_XCAST_MODE_MULTI);
	switch (err) {
	case 0:
		err = 0;
		break;
