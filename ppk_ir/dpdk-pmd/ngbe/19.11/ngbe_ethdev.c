...
RTE_INIT(ngbe_init_log)
{
	...
	ngbe_logtype_rx = rte_log_register("pmd.net.ngbe.rx");
	if (ngbe_logtype_rx >= 0)
		rte_log_set_level(ngbe_logtype_rx, RTE_LOG_DEBUG);
#endif

#ifdef RTE_LIBRTE_NGBE_DEBUG_TX
	ngbe_logtype_tx = rte_log_register("pmd.net.ngbe.tx");
	if (ngbe_logtype_tx >= 0)
		rte_log_set_level(ngbe_logtype_tx, RTE_LOG_DEBUG);
#endif

#ifdef RTE_LIBRTE_NGBE_DEBUG_TX_FREE
	ngbe_logtype_tx_free = rte_log_register("pmd.net.ngbe.tx_free");
	if (ngbe_logtype_tx_free >= 0)
		rte_log_set_level(ngbe_logtype_tx_free, RTE_LOG_DEBUG);
#endif
}
