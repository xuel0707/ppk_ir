...
static int eth_ngbe_dev_init(struct rte_eth_dev *eth_dev);
static int eth_ngbe_dev_uninit(struct rte_eth_dev *eth_dev);
...
static int  ngbe_dev_configure(struct rte_eth_dev *dev);
static int  ngbe_dev_start(struct rte_eth_dev *dev);
static void ngbe_dev_stop(struct rte_eth_dev *dev);
...
static void ngbe_dev_promiscuous_enable(struct rte_eth_dev *dev);
static void ngbe_dev_promiscuous_disable(struct rte_eth_dev *dev);
static void ngbe_dev_allmulticast_enable(struct rte_eth_dev *dev);
static void ngbe_dev_allmulticast_disable(struct rte_eth_dev *dev);
static int ngbe_dev_link_update(struct rte_eth_dev *dev,
				int wait_to_complete);
static void ngbe_dev_stats_get(struct rte_eth_dev *dev,
				struct rte_eth_stats *stats);
static int ngbe_dev_xstats_get(struct rte_eth_dev *dev,
				struct rte_eth_xstat *xstats, unsigned n);
static void ngbe_dev_stats_reset(struct rte_eth_dev *dev);
static void ngbe_dev_xstats_reset(struct rte_eth_dev *dev);
static int ngbe_dev_xstats_get_names(struct rte_eth_dev *dev,
	struct rte_eth_xstat_name *xstats_names,
	unsigned int size);
static int ngbe_dev_queue_stats_mapping_set(struct rte_eth_dev *eth_dev,
					     uint16_t queue_id,
					     uint8_t stat_idx,
					     uint8_t is_rx);
static void ngbe_dev_info_get(struct rte_eth_dev *dev,
			       struct rte_eth_dev_info *dev_info);
static int ngbe_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu);

static int ngbe_vlan_filter_set(struct rte_eth_dev *dev,
		uint16_t vlan_id, int on);
static int ngbe_vlan_tpid_set(struct rte_eth_dev *dev,
			       enum rte_vlan_type vlan_type,
			       uint16_t tpid_id);
static void ngbe_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue,
		int on);
static int ngbe_vlan_offload_config(struct rte_eth_dev *dev, int mask);
static void ngbe_vlan_offload_set(struct rte_eth_dev *dev, int mask);
...
static void ngbe_vlan_hw_extend_enable(struct rte_eth_dev *dev);
static void ngbe_vlan_hw_extend_disable(struct rte_eth_dev *dev);

static int ngbe_dev_led_on(struct rte_eth_dev *dev);
static int ngbe_dev_led_off(struct rte_eth_dev *dev);
static int ngbe_flow_ctrl_get(struct rte_eth_dev *dev,
			       struct rte_eth_fc_conf *fc_conf);
static int ngbe_flow_ctrl_set(struct rte_eth_dev *dev,
			       struct rte_eth_fc_conf *fc_conf);
...
static void ngbe_dev_interrupt_handler(struct rte_intr_handle *handle,
		void *param);
static void ngbe_add_rar(struct rte_eth_dev *dev, struct ether_addr *mac_addr,
			 uint32_t index, uint32_t pool);
static void ngbe_remove_rar(struct rte_eth_dev *dev, uint32_t index);
static void ngbe_set_default_mac_addr(struct rte_eth_dev *dev,
...
static int ngbe_syn_filter_handle(struct rte_eth_dev *dev,
			...
			void *arg);
static int ngbe_add_5tuple_filter(struct rte_eth_dev *dev,
			struct ngbe_5tuple_filter *filter);
static void ngbe_remove_5tuple_filter(struct rte_eth_dev *dev,
			struct ngbe_5tuple_filter *filter);
static int ngbe_ntuple_filter_handle(struct rte_eth_dev *dev,
				enum rte_filter_op filter_op,
				void *arg);
static int ngbe_get_ntuple_filter(struct rte_eth_dev *dev,
			struct rte_eth_ntuple_filter *filter);
static int ngbe_ethertype_filter_handle(struct rte_eth_dev *dev,
				enum rte_filter_op filter_op,
				void *arg);
static int ngbe_get_ethertype_filter(struct rte_eth_dev *dev,
			struct rte_eth_ethertype_filter *filter);
static int ngbe_dev_filter_ctrl(struct rte_eth_dev *dev,
		     enum rte_filter_type filter_type,
		     enum rte_filter_op filter_op,
		     void *arg);

static int ngbe_get_reg_length(struct rte_eth_dev *dev);
static int ngbe_get_regs(struct rte_eth_dev *dev,
			    struct rte_dev_reg_info *regs);
static int ngbe_get_eeprom_length(struct rte_eth_dev *dev);
static int ngbe_get_eeprom(struct rte_eth_dev *dev,
				struct rte_dev_eeprom_info *eeprom);
static int ngbe_set_eeprom(struct rte_eth_dev *dev,
				struct rte_dev_eeprom_info *eeprom);

static int ngbe_timesync_enable(struct rte_eth_dev *dev);
static int ngbe_timesync_disable(struct rte_eth_dev *dev);
static int ngbe_timesync_read_rx_timestamp(struct rte_eth_dev *dev,
					    struct timespec *timestamp,
					    uint32_t flags);
static int ngbe_timesync_read_tx_timestamp(struct rte_eth_dev *dev,
					    struct timespec *timestamp);
static int ngbe_timesync_adjust_time(struct rte_eth_dev *dev, int64_t delta);
static int ngbe_timesync_read_time(struct rte_eth_dev *dev,
				   struct timespec *timestamp);
static int ngbe_timesync_write_time(struct rte_eth_dev *dev,
				   const struct timespec *timestamp);

static int ngbe_dev_l2_tunnel_eth_type_conf
	(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel);
static int ngbe_dev_l2_tunnel_offload_set
 ...
 */
static int
eth_ngbe_dev_init(struct rte_eth_dev *eth_dev)
{
...
static struct eth_driver rte_ngbe_pmd = {
	.pci_drv = {
	...
	.eth_dev_init = eth_ngbe_dev_init,
	.eth_dev_uninit = eth_ngbe_dev_uninit,
	.dev_private_size = sizeof(struct ngbe_adapter),
};
...
#endif /* RTE_VERSION >= RTE_VERSION_NUM(17, 2, 0, 0) */
 ...
 */
static void
ngbe_dev_interrupt_handler(struct rte_intr_handle *handle, void *param)
{
	struct rte_eth_dev *dev = (struct rte_eth_dev *)param;

	ngbe_dev_interrupt_get_status(dev);
	ngbe_dev_interrupt_action(dev, handle);
}
...
RTE_PMD_REGISTER_PCI(net_ngbe, rte_ngbe_pmd.pci_drv);
