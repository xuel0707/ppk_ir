...
static int ngbevf_dev_xstats_get(struct rte_eth_dev *dev,
				  struct rte_eth_xstat *xstats, unsigned n);
static int ngbevf_dev_xstats_get_names(struct rte_eth_dev *dev,
	struct rte_eth_xstat_name *xstats_names, unsigned limit);
static void ngbevf_dev_info_get(struct rte_eth_dev *dev,
				 struct rte_eth_dev_info *dev_info);
/* For Virtual Function support */
static int  ngbevf_dev_configure(struct rte_eth_dev *dev);
static int  ngbevf_dev_start(struct rte_eth_dev *dev);
static int ngbevf_dev_link_update(struct rte_eth_dev *dev,
				   int wait_to_complete);
static void ngbevf_dev_stop(struct rte_eth_dev *dev);
static void ngbevf_dev_close(struct rte_eth_dev *dev);
static void ngbevf_intr_disable(struct rte_eth_dev *dev);
static void ngbevf_intr_enable(struct rte_eth_dev *dev);
static void ngbevf_dev_stats_get(struct rte_eth_dev *dev,
		struct rte_eth_stats *stats);
static void ngbevf_dev_stats_reset(struct rte_eth_dev *dev);
static int ngbevf_vlan_filter_set(struct rte_eth_dev *dev,
		uint16_t vlan_id, int on);
static void ngbevf_vlan_strip_queue_set(struct rte_eth_dev *dev,
		uint16_t queue, int on);
static int ngbevf_vlan_offload_config(struct rte_eth_dev *dev, int mask);
static void ngbevf_vlan_offload_set(struct rte_eth_dev *dev, int mask);
static void ngbevf_set_vfta_all(struct rte_eth_dev *dev, bool on);
static int ngbevf_dev_rx_queue_intr_enable(struct rte_eth_dev *dev,
					    uint16_t queue_id);
static int ngbevf_dev_rx_queue_intr_disable(struct rte_eth_dev *dev,
					     uint16_t queue_id);
static void ngbevf_set_ivar_map(struct ngbe_hw *hw, int8_t direction,
				 uint8_t queue, uint8_t msix_vector);
static void ngbevf_configure_msix(struct rte_eth_dev *dev);
static void ngbevf_dev_promiscuous_enable(struct rte_eth_dev *dev);
static void ngbevf_dev_promiscuous_disable(struct rte_eth_dev *dev);
static void ngbevf_dev_allmulticast_enable(struct rte_eth_dev *dev);
static void ngbevf_dev_allmulticast_disable(struct rte_eth_dev *dev);
static void ngbevf_remove_mac_addr(struct rte_eth_dev *dev, uint32_t index);
static void ngbevf_set_default_mac_addr(struct rte_eth_dev *dev,
					     struct ether_addr *mac_addr);
static int ngbevf_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu);
static int ngbevf_get_reg_length(struct rte_eth_dev *dev);
static int ngbevf_get_regs(struct rte_eth_dev *dev,
				struct rte_dev_reg_info *regs);
static void ngbevf_dev_interrupt_handler(struct rte_intr_handle *handle,
				void *param);
...
ngbevf_negotiate_api(struct ngbe_hw *hw)
{
	...
	static const int sup_ver[] = {
 	...
 	for (i = 0; i < ARRAY_SIZE(sup_ver); i++) {
		if (0 == ngbevf_negotiate_api_version(hw, sup_ver[i]))
			break;
	}
}
...
eth_ngbevf_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	if (err != 0) {
		PMD_INIT_LOG(ERR, "Shared code init failed for ngbevf: %d", err);
		return -EIO;
	}
	...
	eth_dev->data->mac_addrs = rte_zmalloc("ngbevf", ETHER_ADDR_LEN *
 ...
 */
static struct eth_driver rte_ngbevf_pmd = {
	.pci_drv = {
	...
	.eth_dev_init = eth_ngbevf_dev_init,
	.eth_dev_uninit = eth_ngbevf_dev_uninit,
...
ngbevf_intr_enable(struct rte_eth_dev *dev)
{
	...
	/* Save NGBE_VTEIMS value to mask. */
	intr->mask_misc = 0;
}
...
#endif /* RTE_VERSION >= RTE_VERSION_NUM(17, 11, 0, 0) */
		     ...
		     uint8_t queue, uint8_t msix_vector)
{
	...
	if (direction == -1) {
		...
		/* rx or tx cause */
		//msix_vector |= NGBE_VFIVAR_VLD; /* Workround for ICR lost */
...
static void
ngbevf_add_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr,
...
static void
ngbevf_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *addr)
{
	struct ngbe_hw *hw = NGBE_DEV_HW(dev);
...
ngbevf_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu)
{
	...
	if (dev_data->dev_started && !dev_data->scattered_rx &&
	    (max_frame + 2 * NGBE_VLAN_TAG_SIZE >
	 ...
	 * of the mailbox API.
	 */
	ngbevf_rlpml_set_vf(hw, max_frame);
...
ngbevf_dev_promiscuous_enable(struct rte_eth_dev *dev)
{
	...
	hw->mac.update_xcast_mode(hw, NGBEVF_XCAST_MODE_PROMISC);
}
...
ngbevf_dev_promiscuous_disable(struct rte_eth_dev *dev)
{
	...
	hw->mac.update_xcast_mode(hw, NGBEVF_XCAST_MODE_NONE);
}
...
ngbevf_dev_allmulticast_enable(struct rte_eth_dev *dev)
{
	...
	hw->mac.update_xcast_mode(hw, NGBEVF_XCAST_MODE_ALLMULTI);
}
...
ngbevf_dev_allmulticast_disable(struct rte_eth_dev *dev)
{
	...
	hw->mac.update_xcast_mode(hw, NGBEVF_XCAST_MODE_MULTI);
}
...
static void
ngbevf_dev_interrupt_handler(__rte_unused struct rte_intr_handle *handle,
			      void *param)
{
...
RTE_PMD_REGISTER_PCI(net_ngbe_vf, rte_ngbevf_pmd.pci_drv);
