		...
		u32 reg_addr, u32 device_type, u16 *phy_data)
{
	...
	if (!(reg22.page == 0xa43 && (reg22.addr == 0x1a || reg22.addr == 0x1d)))
...
static void ngbe_phy_led_ctrl_rtl(struct ngbe_hw *hw)
{
	...
	if (hw->led_conf != 0xFFFF) {
		...
		if (NGBE_HIGH_LV_EFFECT_GPHY_RGMII == 1)
			value &= 0xFFFC;
		else
			value &= 0xFF8C;
...
s32 ngbe_wait_mdio_access_on(struct ngbe_hw *hw)
{
	int i;
	u16 val = 0;
...
s32 ngbe_efuse_calibration(struct ngbe_hw *hw)
{
	u32 efuse[2];
...
s32 ngbe_init_phy_rtl(struct ngbe_hw *hw)
{
	...
	for (i = 0; i < 15; i++) {
		if (!rd32m(hw, NGBE_STAT, NGBE_STAT_GPHY_IN_RST(hw->bus.lan_id))) {
			break;
		}
		msec_delay(10);
	}
...
s32 ngbe_get_phy_advertised_pause_rtl(struct ngbe_hw *hw, u8 *pause_bit)
{
	...
	status = hw->phy.read_reg(hw, RTL_ANAR, 
			RTL_DEV_ZERO, &value);
...
s32 ngbe_get_phy_lp_advertised_pause_rtl(struct ngbe_hw *hw, u8 *pause_bit)
{
	...
	status = hw->phy.read_reg(hw, RTL_BMSR, 
				RTL_DEV_ZERO, &value);
	value = value & RTL_BMSR_ANC;

	/* if AN complete then check lp adv pause */
	status = hw->phy.read_reg(hw, RTL_ANLPAR, 
				RTL_DEV_ZERO, &value);
	value &= RTL_ANLPAR_LP;
	*pause_bit = (u8)(value >> 10);
	return status;
...
s32 ngbe_set_phy_pause_adv_rtl(struct ngbe_hw *hw, u16 pause_bit)
{
	...
	status = hw->phy.write_reg(hw, RTL_ANAR, 
				RTL_DEV_ZERO, value);
			...
			u32 *speed, bool *link_up)
{
	...
	if (insr & RTL_INSR_ANERR) {
		DEBUGOUT("port %d autoneg error: reg is 0x%x",
			hw->bus.lan_id, insr);
		hw->an_errcnt += 1;
	}
	...
	*speed = NGBE_LINK_SPEED_UNKNOWN;
	 ...
	 */
	status = hw->phy.read_reg(hw, RTL_PHYSR,
				0xa43, &phy_data);
	phy_link = phy_data & RTL_PHYSR_RTLS;
	phy_speed = phy_data & (RTL_PHYSR_SPEED_MASK | 
				RTL_PHYSR_DP);
	if (phy_link == RTL_PHYSR_RTLS) {
		*link_up = true;

		if (phy_speed == (RTL_PHYSR_SPEED_1000M | 
				RTL_PHYSR_DP)) {
			*speed = NGBE_LINK_SPEED_1GB_FULL;
			hw->an_retry = false;
			hw->an_errcnt == 0;
		} else if (phy_speed == (RTL_PHYSR_SPEED_100M |
				RTL_PHYSR_DP)) {
			*speed = NGBE_LINK_SPEED_100M_FULL;
#ifdef RTE_NGBE_RETRY
			ngbe_restart_an_rtl(hw);
#endif
		} else if (phy_speed == (RTL_PHYSR_SPEED_10M |
				RTL_PHYSR_DP)) {
...
#ifdef RTE_NGBE_RETRY
void ngbe_restart_an_rtl(struct ngbe_hw *hw)
{
	if (hw->an_retry || hw->an_errcnt == 0)
		return;

	hw->phy.write_reg(hw, RTL_BMCR, 0, RTL_BMCR_RESTART_AN | RTL_BMCR_ANE);
	DEBUGOUT("port %d an_errcnt = %d, restart AN",
		hw->bus.lan_id, hw->an_errcnt);
	hw->an_retry = true;
}
#endif
