...
static int ngbe_tm_capabilities_get(struct rte_eth_dev *dev,
				     struct rte_tm_capabilities *cap,
				     struct rte_tm_error *error);
static int ngbe_shaper_profile_add(struct rte_eth_dev *dev,
				    uint32_t shaper_profile_id,
				    struct rte_tm_shaper_params *profile,
				    struct rte_tm_error *error);
static int ngbe_shaper_profile_del(struct rte_eth_dev *dev,
				    uint32_t shaper_profile_id,
				    struct rte_tm_error *error);
static int ngbe_node_add(struct rte_eth_dev *dev, uint32_t node_id,
			  uint32_t parent_node_id, uint32_t priority,
			  uint32_t weight, uint32_t level_id,
			  struct rte_tm_node_params *params,
			  struct rte_tm_error *error);
static int ngbe_node_delete(struct rte_eth_dev *dev, uint32_t node_id,
			     struct rte_tm_error *error);
static int ngbe_node_type_get(struct rte_eth_dev *dev, uint32_t node_id,
			       int *is_leaf, struct rte_tm_error *error);
static int ngbe_level_capabilities_get(struct rte_eth_dev *dev,
					uint32_t level_id,
					struct rte_tm_level_capabilities *cap,
					struct rte_tm_error *error);
static int ngbe_node_capabilities_get(struct rte_eth_dev *dev,
				       uint32_t node_id,
				       struct rte_tm_node_capabilities *cap,
				       struct rte_tm_error *error);
static int ngbe_hierarchy_commit(struct rte_eth_dev *dev,
				  int clear_on_fail,
				  struct rte_tm_error *error);

const struct rte_tm_ops ngbe_tm_ops = {
	.capabilities_get = ngbe_tm_capabilities_get,
	.shaper_profile_add = ngbe_shaper_profile_add,
	.shaper_profile_delete = ngbe_shaper_profile_del,
	.node_add = ngbe_node_add,
	.node_delete = ngbe_node_delete,
	.node_type_get = ngbe_node_type_get,
	.level_capabilities_get = ngbe_level_capabilities_get,
	.node_capabilities_get = ngbe_node_capabilities_get,
	.hierarchy_commit = ngbe_hierarchy_commit,
};

int
ngbe_tm_ops_get(struct rte_eth_dev *dev __rte_unused,
		 void *arg)
{
	if (!arg)
		return -EINVAL;

	*(const void **)arg = &ngbe_tm_ops;

	return 0;
}

void
ngbe_tm_conf_init(struct rte_eth_dev *dev)
{
...
void
ngbe_tm_conf_uninit(struct rte_eth_dev *dev)
{
	struct ngbe_tm_conf *tm_conf = NGBE_DEV_TM_CONF(dev);
	struct ngbe_tm_shaper_profile *shaper_profile;
	struct ngbe_tm_node *tm_node;
...
ngbe_tc_nb_get(struct rte_eth_dev *dev)
{
...
static int
ngbe_tm_capabilities_get(struct rte_eth_dev *dev,
			  struct rte_tm_capabilities *cap,
			  struct rte_tm_error *error)
{
	struct ngbe_hw *hw = NGBE_DEV_HW(dev);
	uint8_t tc_nb = ngbe_tc_nb_get(dev);
	 ...
	 */
	/* port + TCs + queues */
	cap->n_nodes_max = 1 + NGBE_DCB_TC_MAX +
...
static inline struct ngbe_tm_shaper_profile *
ngbe_shaper_profile_search(struct rte_eth_dev *dev,
			    uint32_t shaper_profile_id)
{
	struct ngbe_tm_conf *tm_conf = NGBE_DEV_TM_CONF(dev);
...
static int
ngbe_shaper_profile_param_check(struct rte_tm_shaper_params *profile,
				 struct rte_tm_error *error)
{
...
static int
ngbe_shaper_profile_add(struct rte_eth_dev *dev,
			 uint32_t shaper_profile_id,
			 struct rte_tm_shaper_params *profile,
			 struct rte_tm_error *error)
{
	...
	ret = ngbe_shaper_profile_param_check(profile, error);
	if (ret)
		return ret;

	shaper_profile = ngbe_shaper_profile_search(dev, shaper_profile_id);
	...
	shaper_profile = rte_zmalloc("ngbe_tm_shaper_profile",
				     sizeof(struct ngbe_tm_shaper_profile),
				     0);
	if (!shaper_profile)
		return -ENOMEM;
...
static int
ngbe_shaper_profile_del(struct rte_eth_dev *dev,
			 uint32_t shaper_profile_id,
			 struct rte_tm_error *error)
{
	struct ngbe_tm_conf *tm_conf = NGBE_DEV_TM_CONF(dev);
...
static inline struct ngbe_tm_node *
ngbe_tm_node_search(struct rte_eth_dev *dev, uint32_t node_id,
		     enum ngbe_tm_node_type *node_type)
{
	struct ngbe_tm_conf *tm_conf = NGBE_DEV_TM_CONF(dev);
	struct ngbe_tm_node *tm_node;

	if (tm_conf->root && tm_conf->root->id == node_id) {
		*node_type = NGBE_TM_NODE_TYPE_PORT;
		return tm_conf->root;
	}

	TAILQ_FOREACH(tm_node, &tm_conf->tc_list, node) {
		if (tm_node->id == node_id) {
			*node_type = NGBE_TM_NODE_TYPE_TC;
			return tm_node;
		}
	}
	...
	TAILQ_FOREACH(tm_node, &tm_conf->queue_list, node) {
		if (tm_node->id == node_id) {
			*node_type = NGBE_TM_NODE_TYPE_QUEUE;
			return tm_node;
		}
	}
...
static void
ngbe_queue_base_nb_get(struct rte_eth_dev *dev, uint16_t tc_node_no,
			uint16_t *base, uint16_t *nb)
{
	uint8_t nb_tcs = ngbe_tc_nb_get(dev);
...
static int
ngbe_node_param_check(struct rte_eth_dev *dev, uint32_t node_id,
 ...
 */
static int
ngbe_node_add(struct rte_eth_dev *dev, uint32_t node_id,
	       uint32_t parent_node_id, uint32_t priority,
	       uint32_t weight, uint32_t level_id,
	       struct rte_tm_node_params *params,
	       struct rte_tm_error *error)
{
	struct ngbe_tm_conf *tm_conf = NGBE_DEV_TM_CONF(dev);
	enum ngbe_tm_node_type node_type = NGBE_TM_NODE_TYPE_MAX;
	enum ngbe_tm_node_type parent_node_type = NGBE_TM_NODE_TYPE_MAX;
	struct ngbe_tm_shaper_profile *shaper_profile = NULL;
	struct ngbe_tm_node *tm_node;
	struct ngbe_tm_node *parent_node;
	uint8_t nb_tcs;
	uint16_t q_base = 0;
	uint16_t q_nb = 0;
	int ret;
	...
	ret = ngbe_node_param_check(dev, node_id, priority, weight,
				     params, error);
	if (ret)
		return ret;

	/* check if the node ID is already used */
	if (ngbe_tm_node_search(dev, node_id, &node_type)) {
		error->type = RTE_TM_ERROR_TYPE_NODE_ID;
	...
	if (params->shaper_profile_id != RTE_TM_SHAPER_PROFILE_ID_NONE) {
		shaper_profile = ngbe_shaper_profile_search(
	...
	if (parent_node_id == RTE_TM_NODE_ID_NULL) {
		...
		if (level_id != RTE_TM_NODE_LEVEL_ID_ANY &&
		    level_id > NGBE_TM_NODE_TYPE_PORT) {
		...
		/* add the root node */
		tm_node = rte_zmalloc("ngbe_tm_node",
				      sizeof(struct ngbe_tm_node),
				      0);
		if (!tm_node)
			return -ENOMEM;
		tm_node->id = node_id;
	...
	parent_node = ngbe_tm_node_search(dev, parent_node_id,
					   &parent_node_type);
	if (!parent_node) {
		error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
	...
	if (parent_node_type != NGBE_TM_NODE_TYPE_PORT &&
	    parent_node_type != NGBE_TM_NODE_TYPE_TC) {
		error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
	...
	if (parent_node_type == NGBE_TM_NODE_TYPE_PORT) {
		...
		ngbe_queue_base_nb_get(dev, parent_node->no, &q_base, &q_nb);
...
static int
ngbe_node_delete(struct rte_eth_dev *dev, uint32_t node_id,
		  struct rte_tm_error *error)
{
	struct ngbe_tm_conf *tm_conf = NGBE_DEV_TM_CONF(dev);
	enum ngbe_tm_node_type node_type = NGBE_TM_NODE_TYPE_MAX;
	...
	tm_node = ngbe_tm_node_search(dev, node_id, &node_type);
	if (!tm_node) {
		error->type = RTE_TM_ERROR_TYPE_NODE_ID;
	...
	/* root node */
	if (node_type == NGBE_TM_NODE_TYPE_PORT) {
	...
	if (tm_node->shaper_profile)
		tm_node->shaper_profile->reference_count--;
	tm_node->parent->reference_count--;
	if (node_type == NGBE_TM_NODE_TYPE_TC) {
		TAILQ_REMOVE(&tm_conf->tc_list, tm_node, node);
		tm_conf->nb_tc_node--;
	} else {
		TAILQ_REMOVE(&tm_conf->queue_list, tm_node, node);
...
static int
ngbe_node_type_get(struct rte_eth_dev *dev, uint32_t node_id,
		    int *is_leaf, struct rte_tm_error *error)
{
	enum ngbe_tm_node_type node_type = NGBE_TM_NODE_TYPE_MAX;
	struct ngbe_tm_node *tm_node;
	...
	if (node_type == NGBE_TM_NODE_TYPE_QUEUE)
		*is_leaf = true;
	else
		*is_leaf = false;
...
static int
ngbe_level_capabilities_get(struct rte_eth_dev *dev,
			     uint32_t level_id,
			     struct rte_tm_level_capabilities *cap,
			     struct rte_tm_error *error)
{
	...
	if (level_id >= NGBE_TM_NODE_TYPE_MAX) {
	...
	/* root node */
	if (level_id == NGBE_TM_NODE_TYPE_PORT) {
		cap->n_nodes_max = 1;
		cap->n_nodes_nonleaf_max = 1;
		cap->n_nodes_leaf_max = 0;
	} else if (level_id == NGBE_TM_NODE_TYPE_TC) {
		/* TC */
		cap->n_nodes_max = NGBE_DCB_TC_MAX;
		cap->n_nodes_nonleaf_max = NGBE_DCB_TC_MAX;
		cap->n_nodes_leaf_max = 0;
	} else {
		/* queue */
	...
	if (level_id != NGBE_TM_NODE_TYPE_QUEUE) {
...
static int
ngbe_node_capabilities_get(struct rte_eth_dev *dev,
			    uint32_t node_id,
			    struct rte_tm_node_capabilities *cap,
			    struct rte_tm_error *error)
{
...
static int
ngbe_hierarchy_commit(struct rte_eth_dev *dev,
		       int clear_on_fail,
		       struct rte_tm_error *error)
{
	struct ngbe_tm_conf *tm_conf = NGBE_DEV_TM_CONF(dev);
	...
	TAILQ_FOREACH(tm_node, &tm_conf->queue_list, node) {
		...
		if (bw) {
			...
			bw = bw * 8 / 1000 / 1000;
			ret = ngbe_set_queue_rate_limit(dev, tm_node->no, bw);
			if (ret) {
				error->type = RTE_TM_ERROR_TYPE_SHAPER_PROFILE;
				error->message =
...
fail_clear:
	...
	if (clear_on_fail) {
		ngbe_tm_conf_uninit(dev);
		ngbe_tm_conf_init(dev);
	}
	return -EINVAL;
}
