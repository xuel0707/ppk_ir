...
igbuio_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	void *map_addr;
	int err;
    printk(KERN_ERR "igbuio_pci_probe step1\n");
...
#endif
    printk(KERN_ERR "igbuio_pci_probe step2\n");
	 ...
	 * memory
	 */
	 printk(KERN_ERR "igbuio_pci_probe step3\n");
	err = pci_enable_device(dev);
	if (err != 0) {
		dev_err(&dev->dev, "Cannot enable PCI device\n");
		goto fail_free;
	}
    printk(KERN_ERR "igbuio_pci_probe step4\n");
	/* enable bus mastering on the device */
	pci_set_master(dev);

	/* remap IO memory */
	printk(KERN_ERR "igbuio_pci_probe step5\n");
	err = igbuio_setup_bars(dev, &udev->info);
	if (err != 0)
		goto fail_release_iomem;
    printk(KERN_ERR "igbuio_pci_probe step6\n");
	/* set 64-bit DMA mask */
    ...
    printk(KERN_ERR "igbuio_pci_probe step7\n");
	err = pci_set_consistent_dma_mask(dev, DMA_BIT_MASK(64));
	if (err != 0) {
		dev_err(&dev->dev, "Cannot set consistent DMA mask\n");
		goto fail_release_iomem;
	}
    printk(KERN_ERR "igbuio_pci_probe step8\n");
	/* fill uio infos */
	...
	if (err != 0)
		goto fail_release_iomem;
    printk(KERN_ERR "igbuio_pci_probe step9\n");
	/* register uio driver */
	err = uio_register_device(&dev->dev, &udev->info);
	if (err != 0)
		goto fail_remove_group;
     printk(KERN_ERR "igbuio_pci_probe step10\n");   
