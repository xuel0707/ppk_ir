...
#[ uint16_t lkahead_data;
#[ void packet_in_t_lookahead(packet_descriptor_t* pd, int len, uint8_t* in_buf) {
#[    memcpy(&lkahead_data, in_buf, len/8);
#[ }
...
for s in parser.states:
    ...
    for c in s.components:
        ...
        if not c.is_tmp:
            ...
            #[  #if PPK_VLAN
            #[     if (strcmp("${s.name}", "parse_ethernet") == 0) {
            #[       uint32_t* tmp = (uint32_t*)buf;   
            #[       uint32_t vlan1 = *tmp;
            #[       uint32_t vlan = rte_cpu_to_be_32(vlan1);
            #[       uint16_t vlanid = (uint16_t)(vlan >> 16) & 0x0fff;
            #[       uint16_t vlantype = rte_cpu_to_be_16((uint16_t)(vlan)) &0xffff;
            #[       MODIFY_INT32_INT32_BITS_PACKET(pd, header_instance_all_metadatas, field_standard_metadata_t_instance_type, vlantype);
            #[       MODIFY_INT32_INT32_BITS_PACKET(pd, header_instance_ethernet, field_ethernet_t_ether_type, vlantype);
            #[       int ingress_port_value = vlanid - 1000;
            #[       MODIFY_INT32_INT32_BITS_PACKET(pd, header_instance_all_metadatas, field_standard_metadata_t_ingress_port, ingress_port_value);
            #[       debug("       v1model :value of ingress vlan field  " T4LIT(%08x) "\n",GET_INT32_AUTO_PACKET(pd, header_instance_all_metadatas, field_standard_metadata_t_ingress_port));
            #[       pd->parsed_length += 4;
            #[       buf += 4;
            #[     }
            #[  #endif
