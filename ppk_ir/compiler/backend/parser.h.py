...
for hdr in hlir16.header_instances:
    if hasattr(hdr.type._type_ref, 'is_metadata'):
...
global b_isContain_bit_mcast
b_isContain_bit_mcast=False

global b_isContain_recirculate_flag
b_isContain_recirculate_flag=False

#{ enum field_e {
for hdr in hlir16.header_types:
    for fld in hdr.fields:
        if hdr.name == 'standard_metadata_t':
            if fld.name == 'bit_mcast':
                b_isContain_bit_mcast=True
            if fld.name == 'recirculate_flag':
                b_isContain_recirculate_flag=True
        #[   field_${hdr.name}_${fld.name},
if b_isContain_bit_mcast is False:
        #[   field_standard_metadata_t_bit_mcast,
if b_isContain_recirculate_flag is False:
        #[   field_standard_metadata_t_recirculate_flag,
#} };
...
for enum in hlir16.objects['Type_Enum']:
    ...
    if enum.c_name == 'enum_CloneType':
        has_CloneType = False
if has_CloneType:
    #[ enum enum_CloneType { enum_CloneType_I2E, enum_CloneType_E2E };
...
for loc in parser.parserLocals:
    if hasattr(loc.type, 'type_ref'):
        if loc.type.type_ref.node_type == 'Type_Extern':
            #[ ${loc.type.type_ref.name} ${loc.name};
        elif loc.type.type_ref.node_type == 'Type_Header':
            #[ ${loc.type.type_ref.name} ${loc.name};
        else:
            #[ uint8_t ${loc.name}[${loc.type.type_ref.byte_width}]; // type: ${loc.type.type_ref.name}
    else:
        if loc.type.node_type == 'Type_Bits':
            if loc.type.size<=8:
                #[ uint8_t ${loc.name};
            else:
                #[ uint16_t ${loc.name};
