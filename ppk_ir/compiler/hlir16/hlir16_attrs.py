...
def resolve_path_expression_node(hlir16, path_node, parent):
    resolve_lists = []
    if parent.node_type == 'P4Program':
        for obj in hlir16.objects:
            if obj.node_type == 'Declaration_MatchKind':
                continue
            if obj.name == path_node.path.name:
                return obj
...
def get_ctrlloc_sem_name(t,loc):
    if hasattr(t, 'counters'):
        if loc.name == t.counters.expression.path.name:
            return True
    else:
        return False
...
def get_direct_smems(smem_type, tables):
    ...
    return unique_list([(t, loc)
        ...
        if get_ctrlloc_smem_type(loc) == smem_type and  get_ctrlloc_sem_name(t,loc)])
...
def attrs_stateful_memory(hlir16):
    ...
    for table in hlir16.tables:
        ...
        table.direct_meters    = P4Node({}, unique_list([m for t, m in get_direct_smems('direct_meter', [table])]))
        set_common_attrs(hlir16, table.direct_meters)
        if hasattr(table, 'counters'):
            table.direct_counters  = P4Node({}, unique_list([c for t, c in get_direct_smems('direct_counter', [table])]))
    ...
    for t in hlir16.tables:
        if hasattr(t, 'direct_counters'):
            for c in t.direct_counters:
                table_counters.append((t, c))
    hlir16.all_counters = P4Node({}, unique_list(hlir16.counters + table_counters))
    ...
    for _table, smem in hlir16.all_meters + hlir16.all_counters:
        ...
        smem.type.type_ref.name = _table.control.type.name
