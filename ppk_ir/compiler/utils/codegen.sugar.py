...
def gen_format_extern_single(stmt, m, smem_type, is_possibly_multiple, packets_or_bytes = None):
    ...
    extern_type = base_type.path.name
    ...
    externs = {
        ('bytes',   'meter',        'execute_meter'): ( True, [],        ["{0}", "{2}", "{3}"],            ["{0}", "*((uint32_t*)({2}))"]),
        ('packets', 'counter',      'count'):         ( True, [],        ["{0}", "int", "uint32_t"],       ["{0}", "{1}", "1"]),
    ...
    if packets_or_bytes == 'bytes':
        ...
        #pre[ uint32_t $varname = packet_length(pd);
        arg_reformat = ["{0}", "{1}", varname]
...
def fldid(h, f):
    if h._type._type_ref.is_metadata:
        if h.type_ref.name == 'standard_metadata_t':
            inst_type_name = 'standard_metadata'
        else :
            inst_type_name = 'meta'
    elif h.node_type == 'PathExpression':
        inst_type_name = h.ref.name
    else:
        inst_type_name = h.name
...
def gen_format_call_extern(e, mref, method_params):
    ...
    with types({
        ...
        if mref.name == "verify":
            all_params = ", ".join([p for p in [fmt_params, "pd,tables"] if p != ''])
            return_type = format_type(mref.type.returnType)
            param_types = ", ".join([format_type(tpar) for (par, tpar) in method_parameters_by_type(e.arguments, method_params)] + ["packet_descriptor_t* pd, lookup_table_t** tables"])
        else:
