...
def stage_hlir_add_attributes(filename, p4cache):
    return {
        ...
        'loaders': [cache_loader(lambda hlir: set_additional_attrs(hlir, None))],
        'saver': cache_saver(),
    }
...
def load_from_json(compiler_args, cache_dir_name):
    global args
    args = compiler_args
    ...
    with RecursionLimit(10000) as recursion_limit:
        ...
        if hlir is None:
            print("IR compilation failed for file %s" % (os.path.basename(__file__)), file=sys.stderr)
            sys.exit(1)
