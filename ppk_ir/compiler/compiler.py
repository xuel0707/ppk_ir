...
from compiler_load_p4 import load_from_json
...
def generate_code(file, genfile, localvars={}):
    ...
    with open(file, "r") as orig_file:
        ...
        try:
            ...
            # print(code)
            # lianxi(localvars["hlir16"],localvars["generated_code"])
...
def generate_desugared_py():
    ...
    import glob
    for fromfile in glob.glob("compiler/utils/*.sugar.py"):
...
def init_args():
    ...
    #1.create argparse
    parser = argparse.ArgumentParser(description='compile json')
    #2. add arguments to argparse
    parser.add_argument('json', help='The json file')
    parser.add_argument('-c', '--compiler_files_dir', help='Source directory of the compiler\'s files', required=False, default=os.path.join("compiler", "backend"))
...
def write_udef_to_dpdk_model_v1model():
    f = open("ppk/dpdk/data_plane/dpdk_model_v1model.c","r+") 
    lines = f.readlines()
    for line in lines:
        if "//add udef here" in line:
            f.close()
            return
    
    f.write("\n //add udef here")   
    for filename in (f for f in os.listdir("ppk/udef") if f.endswith(".c")):
        newline=("\n"+"#include \"../../udef/{}\"").format(filename)
        f.write(newline)    
    f.close()
...
def generate_files():
    ...
    write_udef_to_dpdk_model_v1model()
...
def main():
    try:
        ...
        global hlir
        hlir= load_from_json(args, cache_dir_name)
