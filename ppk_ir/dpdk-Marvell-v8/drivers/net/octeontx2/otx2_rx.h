...
static __rte_always_inline dpdk_vlib_buffer_t *
nix_get_vlib_from_cqe(void *cq, const uint64_t data_off)
{
	rte_iova_t buff;

	buff = *((rte_iova_t *)((uint64_t *)cq + 9));
	return (dpdk_vlib_buffer_t *)((buff - data_off) +
...
static __rte_always_inline void
vlib_update_ol_flags(const uint64_t rx, uint16_t *ol_flags,
		const void *lookup_mem, uint16_t flag)
{
	if (flag & NIX_RX_OFFLOAD_CHECKSUM_F)
		*ol_flags |= nix_rx_olflags_get(lookup_mem, rx);
